...
public class ColorPalette {
    ...
    private final int[][] table = (int[][]) Array.newInstance((Class<?>) Integer.TYPE, 5, 13);
    ...
    public ColorPalette(ColorScheme colorScheme) {
        ...
        int[] iArr2 = this.table[1];
        int[] iArr3 = this.table[2];
        int[] iArr4 = this.table[3];
        this.table[4][0] = -1;
        iArr4[0] = -1;
        iArr3[0] = -1;
        iArr2[0] = -1;
        iArr[0] = -1;
        for (int i = 0; i < 12; i++) {
            this.table[0][i + 1] = a1[i];
            this.table[1][i + 1] = a2[i];
            this.table[2][i + 1] = a3[i];
            this.table[3][i + 1] = n1[i];
            this.table[4][i + 1] = n2[i];
        }
    }
