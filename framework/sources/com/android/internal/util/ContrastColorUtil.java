...
public class ContrastColorUtil {
    ...
    public boolean isGrayscaleIcon(Drawable d) {
        ...
        if (d instanceof SemPathRenderingDrawable) {
            SemPathRenderingDrawable sd = (SemPathRenderingDrawable) d;
            return sd.getBitmap() != null && isGrayscaleIcon(sd.getBitmap());
        }
        if (d instanceof LayerDrawable) {
            LayerDrawable ld = (LayerDrawable) d;
            int count = ld.getNumberOfLayers();
            return count > 0 && isGrayscaleIcon(ld.getDrawable(0));
        }
        ...
        int count2 = ad.getNumberOfFrames();
        return count2 > 0 && isGrayscaleIcon(ad.getFrame(0));
    }
    ...
    public static boolean isColorDark(int color) {
        return calculateLuminance(color) <= 0.17912878474d;
    }
    ...
    public static int findContrastColor(int color, int other, boolean findFg, double minRatio) {
        ...
        double low = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
        double high = lab[0];
        double a = lab[1];
        double b = lab[2];
    ...
    public static int getShiftedColor(int color, int amount) {
        ...
        if (result[0] >= 4.0d) {
            result[0] = Math.max(SContextConstants.ENVIRONMENT_VALUE_UNKNOWN, result[0] - amount);
        } else {
    ...
    public static boolean shouldInvertTextColor(float opacity, boolean isWhiteWallpaper) {
        if (isWhiteWallpaper || opacity >= 0.25f) {
            return false;
        }
        return true;
    }
