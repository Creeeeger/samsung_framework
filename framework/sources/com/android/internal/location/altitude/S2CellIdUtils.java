...
public final class S2CellIdUtils {
    ...
    private static int xyzToFace(double x, double y, double z) {
        ...
        return absX > absY ? absX > absZ ? x < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 3 : 0 : z < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 5 : 2 : absY > absZ ? y < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 4 : 1 : z < SContextConstants.ENVIRONMENT_VALUE_UNKNOWN ? 5 : 2;
    }

    private static int uToI(double u) {
        double s;
        if (u >= SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
            s = Math.sqrt((3.0d * u) + 1.0d) * 0.5d;
        } else {
            s = 1.0d - (Math.sqrt(1.0d - (3.0d * u)) * 0.5d);
        }
        return Math.max(0, Math.min(View.LAST_APP_AUTOFILL_ID, (int) Math.round((1.073741824E9d * s) - 0.5d)));
    }
