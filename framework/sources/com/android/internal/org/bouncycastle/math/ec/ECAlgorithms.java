...
public class ECAlgorithms {
    ...
    private static ECPoint implSumOfMultiplies(boolean[] zArr, WNafPreCompInfo[] wNafPreCompInfoArr, byte[][] bArr) {
        ...
        while (i3 >= 0) {
            ...
            while (i4 < length) {
                byte[] bArr3 = bArr[i4];
                ?? r12 = i3 < bArr3.length ? bArr3[i3] : z;
                if (r12 != 0) {
                    int abs = Math.abs((int) r12);
                    WNafPreCompInfo wNafPreCompInfo = wNafPreCompInfoArr[i4];
                    eCPoint2 = eCPoint2.add(((r12 < 0 ? true : z) == zArr[i4] ? wNafPreCompInfo.getPreComp() : wNafPreCompInfo.getPreCompNeg())[abs >>> 1]);
                }
                i4++;
                z = false;
            }
