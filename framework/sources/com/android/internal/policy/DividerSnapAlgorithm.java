...
public class DividerSnapAlgorithm {
    ...
    public static DividerSnapAlgorithm create(Context ctx, Rect insets) {
        ...
        if (CoreRune.MW_MULTI_SPLIT_DIVIDER) {
            dividerWindowWidth = ctx.getResources().getDimensionPixelSize(R.dimen.multi_split_screen_divider_thickness);
            if (CoreRune.MW_MULTI_SPLIT_DIVIDER_SIZE_FOLD) {
                dividerInsets = ctx.getResources().getDimensionPixelSize(R.dimen.multi_split_screen_divider_insets_fold);
            } else {
                dividerInsets = ctx.getResources().getDimensionPixelSize(R.dimen.multi_split_screen_divider_insets);
            }
        }
    ...
    public DividerSnapAlgorithm(Resources res, int displayWidth, int displayHeight, int dividerSize, boolean isHorizontalDivision, Rect insets, int dockSide, boolean isMinimizedMode, boolean isHomeResizable) {
        ...
        this.mMultiSplitMinimalSize = res.getDimensionPixelSize(R.dimen.multi_split_minimal_size_resizable_task);
        this.mMinimumSplitTargetGap = res.getDimensionPixelSize(R.dimen.multi_split_minimum_target_gap);
    ...
    public boolean isSplitScreenFeasible() {
        ...
        return availableSpace / 2 >= (CoreRune.MW_MULTI_SPLIT_SNAP_ALGORITHM ? getMinimalSize() : this.mMinimalSizeResizableTask);
    }
    ...
    private int getMinimalSize() {
        ...
        if (MultiWindowUtils.isTablet() || (CoreRune.MW_MULTI_SPLIT_FOR_COVER_DISPLAY && this.mDisplayDeviceType == 5)) {
            i = 420;
        } else {
            i = 460;
        }
    ...
    public int getTargetMinimalRatio() {
        float ratio;
        if (this.mIsHorizontalDivision) {
            ratio = this.mMultiSplitMinimalSize / ((this.mDisplayHeight - this.mInsets.top) - this.mInsets.bottom);
        } else {
            ratio = this.mMultiSplitMinimalSize / ((this.mDisplayWidth - this.mInsets.left) - this.mInsets.right);
        }
