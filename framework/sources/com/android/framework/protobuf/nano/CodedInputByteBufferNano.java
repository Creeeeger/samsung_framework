...
public final class CodedInputByteBufferNano {
    ...
    public int readRawVarint32() throws IOException {
        ...
        if (tmp4 >= 0) {
            return result3 | (tmp4 << SprAnimatorBase.INTERPOLATOR_TYPE_CUBICEASEINOUT);
        }
        int result4 = result3 | ((tmp4 & Byte.MAX_VALUE) << 21);
        byte tmp5 = readRawByte();
        int result5 = result4 | (tmp5 << SprAnimatorBase.INTERPOLATOR_TYPE_QUADEASEIN);
        if (tmp5 < 0) {
            for (int i = 0; i < 5; i++) {
                if (readRawByte() >= 0) {
                    return result5;
