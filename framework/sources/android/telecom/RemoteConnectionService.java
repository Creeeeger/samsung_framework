...
final class RemoteConnectionService {
    ...
    final RemoteConnection createRemoteConnection(PhoneAccountHandle connectionManagerPhoneAccount, ConnectionRequest request, boolean isIncoming) {
        ...
        if (!this.mTelecomFeatureFlags.setRemoteConnectionCallId()) {
            shouldShowIncomingUI = false;
        } else {
            boolean shouldShowIncomingUI2 = request.shouldShowIncomingCallUi();
            shouldShowIncomingUI = shouldShowIncomingUI2;
        }
        String telecomCallId = request.getTelecomCallId();
        ConnectionRequest newRequest = new ConnectionRequest.Builder().setAccountHandle(request.getAccountHandle()).setAddress(request.getAddress()).setExtras(extras).setVideoState(request.getVideoState()).setRttPipeFromInCall(request.getRttPipeFromInCall()).setRttPipeToInCall(request.getRttPipeToInCall()).setTelecomCallId(telecomCallId).setShouldShowIncomingCallUi(shouldShowIncomingUI).build();
        try {
    ...
    RemoteConference createRemoteConference(PhoneAccountHandle connectionManagerPhoneAccount, ConnectionRequest request, boolean isIncoming) {
        final String id = UUID.randomUUID().toString();
        try {
            if (this.mConferenceById.isEmpty()) {
                this.mOutgoingConnectionServiceRpc.addConnectionServiceAdapter(this.mServant.getStub(), null);
            }
            String telecomCallId = this.mTelecomFeatureFlags.setRemoteConnectionCallId() ? id : request.getTelecomCallId();
            ConnectionRequest newRequest = new ConnectionRequest.Builder().setAccountHandle(request.getAccountHandle()).setAddress(request.getAddress()).setExtras(request.getExtras()).setVideoState(request.getVideoState()).setShouldShowIncomingCallUi(request.shouldShowIncomingCallUi()).setRttPipeFromInCall(request.getRttPipeFromInCall()).setRttPipeToInCall(request.getRttPipeToInCall()).setParticipants(request.getParticipants()).setIsAdhocConferenceCall(request.isAdhocConferenceCall()).setTelecomCallId(telecomCallId).build();
            RemoteConference conference = new RemoteConference(id, this.mOutgoingConnectionServiceRpc);
            this.mOutgoingConnectionServiceRpc.createConference(connectionManagerPhoneAccount, id, newRequest, isIncoming, false, null);
