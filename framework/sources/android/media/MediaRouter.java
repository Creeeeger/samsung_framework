...
public class MediaRouter {
    ...
    static class Static implements DisplayManager.DisplayListener {
        ...
        void updateAudioRoutes(AudioRoutesInfo newRoutes) {
            ...
            if (!TextUtils.equals(newRoutes.bluetoothName, this.mCurAudioRoutesInfo.bluetoothName)) {
                ...
                audioRoutesChanged = true;
            } else if (TextUtils.equals(newRoutes.bluetoothName, this.mCurAudioRoutesInfo.bluetoothName) && this.mSelectedRoute != this.mBluetoothA2dpRoute && isBluetoothA2dpOn()) {
    ...
    static void selectRouteStatic(int types, RouteInfo route, boolean explicit) {
        ...
        if (activeDisplay != null || oldRouteHasAddress || newRouteHasAddress) {
            ...
            } else if (activeDisplay != null && !newRouteHasAddress && oldRouteHasAddress && activeDisplay.getDeviceAddress().equals(oldRoute.mDeviceAddress)) {
    ...
    static void updateWifiDisplayStatus(WifiDisplayStatus status) {
        ...
        WifiDisplay d;
        if (status.getFeatureState() == 3 && status.getConnectedState() != 3) {
    ...
    static boolean isAudioPathA2DPStatic() {
        ...
        try {
            device = sStatic.mAudioService.getDeviceMaskForStream(stream);
        } catch (RemoteException e) {
            device = AudioSystem.getDevicesForStream(stream);
        }
        if (((device - 1) & device) != 0) {
            if ((device & 2) != 0) {
                device = 2;
            } else {
                for (Integer it : AudioSystem.DEVICE_OUT_ALL_A2DP_SET) {
                    device &= it.intValue();
                }
            }
        }
        return AudioSystem.DEVICE_OUT_ALL_A2DP_SET.contains(Integer.valueOf(device));
    }
}
