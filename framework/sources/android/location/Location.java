...
public class Location implements Parcelable {
    ...
    public static double convert(String coordinate) {
        ...
        try {
            ...
            try {
                ...
                double sec = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
    ...
    private static void computeDistanceAndBearing(double lat1, double lon1, double lat2, double lon2, BearingDistanceCache results) {
        ...
        double sigma = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
        double deltaSigma = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
        double cosLambda = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
        double sinLambda = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
        double lambda = l;
        int iter = 0;
        while (true) {
            if (iter >= 20) {
            ...
            double cos2SM = SContextConstants.ENVIRONMENT_VALUE_UNKNOWN;
            if (sinSigma2 == SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
                sinAlpha = 0.0d;
            } else {
                sinAlpha = (cosU1cosU2 * sinLambda) / sinSigma2;
            }
            double cosSqAlpha = 1.0d - (sinAlpha * sinAlpha);
            if (cosSqAlpha != SContextConstants.ENVIRONMENT_VALUE_UNKNOWN) {
