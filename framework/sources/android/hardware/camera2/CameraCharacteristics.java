...
public final class CameraCharacteristics extends CameraMetadata<Key<?>> {
    ...
    @SyntheticKey
    @PublicKey
    public static final Key<MandatoryStreamCombination[]> SCALER_MANDATORY_PREVIEW_STABILIZATION_OUTPUT_STREAM_COMBINATIONS = new Key<>("android.scaler.mandatoryPreviewStabilizationOutputStreamCombinations", MandatoryStreamCombination[].class);
    ...
    public <TKey> List<TKey> semGetAvailableSamsungKeyList(Class<?> metadataClass, Class<TKey> keyClass, Key<int[]> key) {
        int[] filterTags = (int[]) get(key);
        if (filterTags == null) {
            return null;
        }
        List<TKey> availableSamsungKeys = getAvailableKeyList(metadataClass, keyClass, filterTags, false);
