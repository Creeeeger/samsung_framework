...
public abstract class SensorManager {
    ...
    public Sensor getDefaultSensor(int type) {
        ...
        if (type == 65601) {
            ...
            for (Sensor sensor3 : getSensorList(5)) {
                if (sensor3.isWakeUpSensor() == wakeUpSensor) {
        ...
        if (type > 65536) {
            List<Sensor> p = getSensorList(type);
            if (p.size() > 0) {
                Sensor sensor4 = p.get(0);
                Log.i(TAG, "Get Sensor type = " + type);
                return sensor4;
            }
            return null;
        }
    ...
    public void setActiveScreenParameter(int screenID) {
        List<Sensor> l = getSensorList(Sensor.TYPE_SCONTEXT);
    ...
    public void setBrightnessHysteresisParameter(float[] values) {
        List<Sensor> l = getSensorList(5);
        for (Sensor sensor : l) {
