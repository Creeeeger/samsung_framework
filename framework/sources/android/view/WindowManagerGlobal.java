...
public final class WindowManagerGlobal {
    ...
    public static final int ADD_FLAG_BOUNDS_COMPAT_MODE = 1048576;
    public static final int ADD_FLAG_BOUNDS_COMPAT_TRANSLATOR = 8388608;
    ...
    public static final int RELAYOUT_RES_ALLOW_POKE_DRAW_LOCK = 1024;
    public static final int RELAYOUT_RES_BOUNDS_COMPAT_MODE = 1048576;
    public static final int RELAYOUT_RES_BOUNDS_COMPAT_TRANSLATOR = 8388608;
    ...
    public void addView(View view, ViewGroup.LayoutParams params, Display display, Window parentWindow, int userId) {
        ...
        if (this.mViews.size() >= 300) {
            synchronized (this.mLock) {
                int i2 = this.mViews.size() - 1;
                for (int j = 0; i2 >= 0 && j < 50; j++) {
                    Log.d(TAG, "addedView(" + i2 + NavigationBarInflaterView.KEY_CODE_END + this.mViews.get(i2));
                    if (this.mParams.get(i2) != null) {
                        Log.d(TAG, "addedParams(" + i2 + NavigationBarInflaterView.KEY_CODE_END + this.mParams.get(i2) + " / Title: " + ((Object) this.mParams.get(i2).getTitle()));
                    }
                    i2--;
                }
            }
        ...
        synchronized (this.mLock) {
            ...
            if (wparams.type >= 1000 && wparams.type <= 1999) {
                ...
                for (int i3 = 0; i3 < count; i3++) {
                    if (this.mRoots.get(i3).mWindow.asBinder() == wparams.token) {
                        panelParentView = this.mViews.get(i3);
                    }
                }
            }
            ...
            if (wparams.token != null && panelParentView == null) {
                int i4 = 0;
                while (true) {
                    if (i4 >= this.mWindowlessRoots.size()) {
                        break;
                    }
                    ViewRootImpl maybeParent = this.mWindowlessRoots.get(i4);
