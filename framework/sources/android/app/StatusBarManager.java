...
public class StatusBarManager {
    ...
    public static int getNaturalBarTypeByDisplayId(Context context, int displayId) {
        if (displayId == 0) {
            if (!context.getResources().getConfiguration().isDexMode()) {
                return 0;
            }
            return 1;
        }
    ...
    public void expandSettingsPanel(String subPanel) {
        try {
            IStatusBarService svc = getService();
            if (svc != null) {
                svc.expandSettingsPanelToType(subPanel, getBarTypeFromContext());
            }
        } catch (RemoteException ex) {
            throw ex.rethrowFromSystemServer();
        }
    ...
    public void disableToType(int what, int barType) {
        try {
            int userId = Binder.getCallingUserHandle().getIdentifier();
            IStatusBarService svc = getService();
            if (svc != null) {
    ...
    public void disable2ToType(int what, int barType) {
        try {
            int userId = Binder.getCallingUserHandle().getIdentifier();
            IStatusBarService svc = getService();
            if (svc != null) {
