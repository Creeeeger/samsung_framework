...
public final class WallpaperColors implements Parcelable {
    ...
    public WallpaperColors(Map<Integer, Integer> colorToPopulation, int colorHints) {
        ...
        Iterator<Integer> it = colorToPopulation.keySet().iterator();
        while (it.hasNext()) {
        ...
        while (it2.hasNext()) {
            Map.Entry<Integer, Double> colorToScoreEntry = it2.next();
            colorsByScoreDescending.add(colorToScoreEntry.getKey());
        }
        List<Integer> mainColorInts = new ArrayList<>();
        Iterator<Integer> it3 = colorsByScoreDescending.iterator();
        while (it3.hasNext()) {
            int color3 = it3.next().intValue();
            Cam cam = colorToCam.get(Integer.valueOf(color3));
            Iterator<Integer> it4 = mainColorInts.iterator();
            while (true) {
                if (it4.hasNext()) {
                    int otherColor = it4.next().intValue();
        ...
        Iterator<Integer> it5 = mainColorInts.iterator();
        while (it5.hasNext()) {
            int colorInt = it5.next().intValue();
