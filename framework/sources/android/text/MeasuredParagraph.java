...
public class MeasuredParagraph {
    ...
    private void resetAndAnalyzeBidi(CharSequence text, int start, int end, TextDirectionHeuristic textDir) {
        ...
        int bidi = AndroidBidi.bidi(bidiRequest, this.mCopiedBuffer, this.mLevels.getRawArray());
        this.mParaDir = bidi;
        this.mLtrWithoutBidi = false;
        if (bidi == -1) {
            String newStr = new String(this.mCopiedBuffer, 0, this.mTextLength);
            Matcher m = phoneNum.matcher(newStr);
            while (m.find()) {
                int startIndex = m.start();
                if (startIndex >= 0 && startIndex < this.mTextLength) {
                    AutoGrowArray.ByteArray byteArray = this.mLevels;
                    byteArray.set(startIndex, byteArray.get(startIndex + 1));
                }
            }
        }
    }
