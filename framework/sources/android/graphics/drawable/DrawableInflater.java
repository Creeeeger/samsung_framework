...
public final class DrawableInflater {
    ...
    Drawable inflateFromXmlForDensity(String name, XmlPullParser parser, AttributeSet attrs, int density, Resources.Theme theme) throws XmlPullParserException, IOException {
        ...
        Drawable drawable = inflateSpr(name, parser, attrs);
        if (drawable == null) {
    ...
    private Drawable inflateFromTag(String name) {
        ...
        switch (name.hashCode()) {
            case -2024464016:
                ...
                break;
            case -1724158635:
                if (name.equals(SemLocationManager.GEOFENCE_TRANSITION)) {
                    c = 4;
                    break;
                }
                c = 65535;
    ...
    private Drawable inflateSpr(String name, XmlPullParser parser, AttributeSet attrs) throws XmlPullParserException, IOException {
        ...
        if (id != 0) {
            ...
            byte[] b = new byte[3];
            try {
                is = this.mRes.openRawResource(id);
                is.read(b, 0, 3);
                if (b[0] == 83 && b[1] == 80 && b[2] == 82) {
                    z = true;
                }
                isSpr = z;
            } finally {
