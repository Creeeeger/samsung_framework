...
public class Environment {
    ...
    public static Collection<File> getInternalMediaDirectories() {
        ...
        String omcPath = SystemProperties.get(AsProperty.PROP_CFG_OMC_RESPATH, "");
        if (omcPath.length() > 0) {
            addCanonicalFile(res, new File(omcPath));
        }
        addCanonicalFile(res, new File(getDataDirectory(), "overlays/media"));
        return res;
    }
