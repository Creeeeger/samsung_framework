...
public class TrustArchive {
    ...
    public void dump(PrintWriter printWriter, int i, int i2, String str, boolean z) {
        int i3;
        Iterator descendingIterator = this.mEvents.descendingIterator();
        int i4 = 0;
        while (descendingIterator.hasNext() && i4 < i) {
            Event event = (Event) descendingIterator.next();
            if (i2 == -1 || i2 == (i3 = event.userId) || i3 == -1) {
                printWriter.print(str);
                printWriter.printf("#%-2d %s %s: ", Integer.valueOf(i4), formatElapsed(event.elapsedTimestamp), dumpType(event.type));
                if (i2 == -1) {
                    printWriter.print("user=");
                    printWriter.print(event.userId);
                    printWriter.print(", ");
                }
                if (event.agent != null) {
                    printWriter.print("agent=");
                    if (z) {
                        printWriter.print(event.agent.flattenToShortString());
                    } else {
                        printWriter.print(getSimpleName(event.agent));
                    }
                }
                int i5 = event.type;
                if (i5 == 0) {
                    printWriter.printf(", message=\"%s\", duration=%s, flags=%s", event.message, formatDuration(event.duration), dumpGrantFlags(event.flags));
                } else if (i5 == 6) {
                    printWriter.printf(", managingTrust=" + event.managingTrust, new Object[0]);
                }
                printWriter.println();
                i4++;
            }
