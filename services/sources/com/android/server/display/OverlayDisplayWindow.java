...
public final class OverlayDisplayWindow implements DumpUtils.Dump {
    ...
    public final void resize(int i, int i2, int i3, boolean z) {
        ...
        this.mHeight = i2;
        this.mDensityDpi = i3;
        this.mTitle = this.mContext.getResources().getString(R.string.lockscreen_sound_off_label, this.mName, Integer.valueOf(this.mWidth), Integer.valueOf(this.mHeight), Integer.valueOf(this.mDensityDpi));
        if (this.mSecure) {
            this.mTitle += this.mContext.getResources().getString(R.string.lockscreen_sim_unlock_progress_dialog_message);
        }
        if (z) {
            relayout();
        }
    ...
    public final void createWindow() {
        View inflate = LayoutInflater.from(this.mContext).inflate(R.layout.screen, (ViewGroup) null);
        this.mWindowContent = inflate;
        inflate.setOnTouchListener(this.mOnTouchListener);
        TextureView textureView = (TextureView) this.mWindowContent.findViewById(R.id.small);
        this.mTextureView = textureView;
        textureView.setPivotX(DisplayPowerController2.RATE_FROM_DOZE_TO_ON);
        this.mTextureView.setPivotY(DisplayPowerController2.RATE_FROM_DOZE_TO_ON);
        ...
        TextView textView = (TextView) this.mWindowContent.findViewById(R.id.smallIcon);
        ...
        if (CoreRune.MD_DEX_EMULATOR && isDexEmulatorWindow()) {
            this.mWindowParams.flags |= 128;
        }
        WindowManager.LayoutParams layoutParams2 = this.mWindowParams;
        layoutParams2.privateFlags |= 2;
        layoutParams2.alpha = 0.8f;
        layoutParams2.gravity = 51;
        layoutParams2.setTitle(this.mTitle);
    ...
    public final void clearLiveState() {
        this.mLiveTranslationX = DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
        this.mLiveTranslationY = DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
        this.mLiveScale = 1.0f;
    }

    public final boolean isDexEmulatorWindow() {
        String[] split;
        String string = Settings.Global.getString(this.mContext.getContentResolver(), "overlay_display_devices");
        return string != null && (split = string.split("#")) != null && split.length == 3 && split[0].equals("dex");
    }
}
