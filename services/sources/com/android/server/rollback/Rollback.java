...
public class Rollback {
    ...
    public static void addAll(List list, int[] iArr) {
        for (int i : iArr) {
    ...
    public void commit(final Context context, final List list, String str, final IntentSender intentSender) {
        boolean z;
        File[] fileArr;
    ...
    public List getApexPackageNames() {
        assertInWorkerThread();
        ArrayList arrayList = new ArrayList();
        for (PackageRollbackInfo packageRollbackInfo : this.info.getPackages()) {
            if (packageRollbackInfo.isApex()) {
    ...
    public static int rollbackStateFromString(String str) {
        str.hashCode();
        char c = 65535;
        switch (str.hashCode()) {
            case -1491142788:
    ...
    public static boolean extensionVersionReductionWouldViolateConstraint(SparseIntArray sparseIntArray, PackageManagerInternal packageManagerInternal) {
        ...
        List packageNames = packageManagerInternal.getPackageList().getPackageNames();
        for (int i = 0; i < packageNames.size(); i++) {
            SparseIntArray minExtensionVersions = packageManagerInternal.getPackage((String) packageNames.get(i)).getMinExtensionVersions();
