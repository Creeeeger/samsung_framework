...
public final class ProcessServiceRecord {
    ...
    public boolean areAllShortForegroundServicesProcstateTimedOut(long j) {
        ...
        for (int size = this.mServices.size() - 1; size >= 0; size--) {
            ...
            if (serviceRecord.isShortFgs() && serviceRecord.hasShortFgsInfo() && serviceRecord.getShortFgsInfo().getProcStateDemoteTime() >= j) {
                return false;
            }
        }
        return true;
    ...
    public int getNumForegroundServices() {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            if (((ServiceRecord) this.mServices.valueAt(i2)).isForeground) {
                i++;
            }
        }
        return i;
    ...
    public final boolean isAlmostPerceptible(ServiceRecord serviceRecord) {
        ...
        for (int size = connections.size() - 1; size >= 0; size--) {
            ...
            for (int size2 = arrayList.size() - 1; size2 >= 0; size2--) {
                if (((ConnectionRecord) arrayList.get(size2)).hasFlag(65536)) {
                    return true;
                }
            }
        }
    ...
    public void updateHasAboveClientLocked() {
        ...
        for (int size = this.mConnections.size() - 1; size >= 0; size--) {
            if (((ConnectionRecord) this.mConnections.valueAt(size)).hasFlag(8)) {
    ...
    public ServiceRecord getExecutingServiceAt(int i) {
        return (ServiceRecord) this.mExecutingServices.valueAt(i);
    }
    ...
    public void updateBoundClientUids() {
        ...
        for (int i = 0; i < size; i++) {
            ArrayMap connections = ((ServiceRecord) this.mServices.valueAt(i)).getConnections();
            int size2 = connections.size();
            for (int i2 = 0; i2 < size2; i2++) {
                ArrayList arrayList = (ArrayList) connections.valueAt(i2);
