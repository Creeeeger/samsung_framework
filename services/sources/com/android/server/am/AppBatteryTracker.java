...
public final class AppBatteryTracker extends BaseAppStateTracker implements AppRestrictionController.UidBatteryUsageProvider {
    ...
    public final void checkBatteryUsageStats() {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                appBatteryPolicy.handleUidBatteryUsage(keyAt, immutableBatteryUsage.mutate().subtract(this.mAppRestrictionController.getUidBatteryExemptedUsageSince(keyAt, max, elapsedRealtime, appBatteryPolicy.mBgCurrentDrainExemptedTypes)).calcPercentage(keyAt, appBatteryPolicy).unmutate());
            }
        ...
        synchronized (this.mLock) {
            ...
            if (sparseArray.size() == 0) {
                ...
                for (int size = sparseArray.size(); i < size; size = size) {
                    ...
                    printWriter.format("%s%s: [%s] %s (%s) | %s (%s) | %s (%s) | %s\n", str2, UserHandle.formatUid(keyAt), PowerExemptionManager.reasonCodeToString(appBatteryPolicy.shouldExemptUid(keyAt)), calcPercentage.toString(), calcPercentage.percentageToString(), calcPercentage2.toString(), calcPercentage2.percentageToString(), calcPercentage3.toString(), calcPercentage3.percentageToString(), ((BatteryUsage) this.mUidBatteryUsage.get(keyAt, BATTERY_USAGE_NONE)).toString());
                    i++;
                }
            }
        }
    ...
    public final class AppBatteryPolicy extends BaseAppStatePolicy {
        ...
        public AppBatteryPolicy(BaseAppStateTracker.Injector injector, AppBatteryTracker appBatteryTracker) {
            super(injector, appBatteryTracker, "bg_current_drain_monitor_enabled", appBatteryTracker.mContext.getResources().getBoolean(R.bool.config_customUserSwitchUi));
            ...
            float[] floatArray = getFloatArray(resources.obtainTypedArray(R.array.vendor_required_apps_managed_profile));
            float f = ActivityManager.isLowRamDeviceStatic() ? floatArray[1] : floatArray[0];
            this.mDefaultBgCurrentDrainRestrictedBucket = f;
            float[] floatArray2 = getFloatArray(resources.obtainTypedArray(R.array.vendor_required_apps_managed_device));
            ...
            long integer = resources.getInteger(R.integer.config_defaultRefreshRateInZone) * 1000;
            this.mDefaultBgCurrentDrainWindowMs = integer;
            this.mDefaultBgCurrentDrainInteractionGracePeriodMs = integer;
            float[] floatArray3 = getFloatArray(resources.obtainTypedArray(R.array.vendor_disallowed_apps_managed_user));
            ...
            float[] floatArray4 = getFloatArray(resources.obtainTypedArray(R.array.vendor_disallowed_apps_managed_profile));
            ...
            long integer2 = resources.getInteger(R.integer.config_defaultNotificationVibrationIntensity) * 1000;
            this.mDefaultBgCurrentDrainMediaPlaybackMinDuration = integer2;
            long integer3 = resources.getInteger(R.integer.config_defaultNotificationLedOn) * 1000;
            this.mDefaultBgCurrentDrainLocationMinDuration = integer3;
            this.mDefaultBgCurrentDrainEventDurationBasedThresholdEnabled = resources.getBoolean(R.bool.config_closeDialogWhenTouchOutside);
            this.mDefaultBgCurrentDrainAutoRestrictAbusiveAppsEnabled = resources.getBoolean(R.bool.config_checkWallpaperAtBoot);
            this.mDefaultCurrentDrainTypesToRestrictedBucket = resources.getInteger(R.integer.config_defaultRefreshRate);
            this.mDefaultBgCurrentDrainTypesToBgRestricted = resources.getInteger(R.integer.config_defaultPictureInPictureGravity);
            this.mDefaultBgCurrentDrainPowerComponent = resources.getInteger(R.integer.config_defaultPeakRefreshRate);
            this.mDefaultBgCurrentDrainExemptedTypes = resources.getInteger(R.integer.config_defaultNotificationLedOff);
            this.mDefaultBgCurrentDrainHighThresholdByBgLocation = resources.getBoolean(R.bool.config_customBugreport);
