...
public final class ReceiverList extends ArrayList implements IBinder.DeathRecipient {
    ...
    public boolean containsFilter(IntentFilter intentFilter) {
        ...
        for (int i = 0; i < size; i++) {
            if (IntentFilter.filterEquals((BroadcastFilter) get(i), intentFilter)) {
                return true;
            }
        }
        return false;
    ...
    public void dumpDebug(ProtoOutputStream protoOutputStream, long j) {
        ...
        int size = size();
        for (int i = 0; i < size; i++) {
            ((BroadcastFilter) get(i)).dumpDebug(protoOutputStream, 2246267895815L);
        }
    ...
    public void dump(PrintWriter printWriter, String str) {
        ...
        int size = size();
        for (int i = 0; i < size; i++) {
            BroadcastFilter broadcastFilter = (BroadcastFilter) get(i);
            printWriter.print(str);
