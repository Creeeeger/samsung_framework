...
public final class AppFGSTracker extends BaseAppStateDurationsTracker implements ActivityManagerInternal.ForegroundServiceStateListener {
    ...
    public AppFGSTracker(Context context, AppRestrictionController appRestrictionController, Constructor constructor, Object obj) {
    ...
    public final void checkLongRunningFgs() {
        ...
        final ArrayMap arrayMap = this.mTmpPkgDurations;
        long elapsedRealtime = SystemClock.elapsedRealtime();
        ...
        synchronized (this.mLock) {
            ...
            while (size >= 0) {
                ...
                while (size2 >= 0) {
                    ...
                    if (packageDurations.hasForegroundServices() && packageDurations.getForegroundServiceType() == 0) {
                        j = fgsLongRunningThreshold;
                        MARsHandler.getInstance().sendFGSRestrictionTargetMsgToMainHandler(packageDurations.mPackageName, packageDurations.mUid);
                    } else {
                        j = fgsLongRunningThreshold;
                    }
                    size2--;
                    fgsLongRunningThreshold = j;
                }
                size--;
                i = 1;
            }
        ...
        if (size3 > 0) {
            ...
            for (int i3 = size3 - 1; i3 >= 0; i3--) {
                PackageDurations packageDurations2 = (PackageDurations) arrayMap.keyAt(numArr[i3].intValue());
                appFGSPolicy.onLongRunningFgs(packageDurations2.mPackageName, packageDurations2.mUid, packageDurations2.mExemptReason);
            }
            arrayMap.clear();
        }
        synchronized (this.mLock) {
    ...
    public boolean hasForegroundServiceNotifications(int i) {
        synchronized (this.mLock) {
            ArrayMap arrayMap = (ArrayMap) this.mFGSNotificationIDs.getMap().get(i);
            if (arrayMap != null) {
                for (int size = arrayMap.size() - 1; size >= 0; size--) {
                    if (hasForegroundServiceNotificationsLocked((String) arrayMap.keyAt(size), i)) {
                        return true;
                    }
                }
            }
    ...
    public void dumpOthers(PrintWriter printWriter, String str) {
        ...
        String str2 = "  " + str;
        synchronized (this.mLock) {
            SparseArray map = this.mFGSNotificationIDs.getMap();
            if (map.size() == 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                ArrayMap arrayMap = (ArrayMap) map.valueAt(i);
                ...
                for (int i2 = 0; i2 < size2; i2++) {
                    ...
                    printWriter.println(hasForegroundServiceNotificationsLocked(str3, keyAt));
                }
            }
        }
    }
    ...
    /* loaded from: classes.dex */
    public class PackageDurations extends BaseAppStateDurations {
        ...
        public void addEvent(boolean z, long j) {
            ...
            while (true) {
                ...
                if (i < linkedListArr.length) {
                    if (linkedListArr[i] != null && isActive(i)) {
        ...
        public int getForegroundServiceType() {
            return this.mForegroundServiceTypes;
        }
