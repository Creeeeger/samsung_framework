...
public final class RemotePrintService implements IBinder.DeathRecipient {
    ...
    public interface PrintServiceCallbacks {
        ...
        void onPrintersAdded(List list);

        void onPrintersRemoved(List list);
    ...
    public final void handleStartPrinterDiscovery(final List list) {
    ...
    public final void handleValidatePrinters(final List list) {
        if (!isBound()) {
            ensureBound();
    ...
    public final void stopTrackingAllPrinters() {
        synchronized (this.mLock) {
            ...
            for (int size = list.size() - 1; size >= 0; size--) {
                PrinterId printerId = (PrinterId) this.mTrackedPrinterList.get(size);
        ...
        synchronized (this.mLock) {
            ...
            if (list != null) {
                ...
                for (int i = 0; i < size; i++) {
                    com.android.internal.print.DumpUtils.writePrinterId(dualDumpOutputStream, "tracked_printers", 2246267895815L, (PrinterId) this.mTrackedPrinterList.get(i));
                }
            }
        }
    }
    ...
    public final class RemotePrintServiceClient extends IPrintServiceClient.Stub {
        ...
        public final void throwIfPrinterIdsForPrinterInfoTampered(ComponentName componentName, List list) {
            int size = list.size();
            for (int i = 0; i < size; i++) {
        ...
        public final void throwIfPrinterIdsTampered(ComponentName componentName, List list) {
            int size = list.size();
            for (int i = 0; i < size; i++) {
