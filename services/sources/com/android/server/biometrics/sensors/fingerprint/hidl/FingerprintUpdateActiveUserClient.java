...
public class FingerprintUpdateActiveUserClient extends HalClientMonitor {
    ...
    public FingerprintUpdateActiveUserClient(Context context, Supplier supplier, int i, String str, int i2, BiometricLogger biometricLogger, BiometricContext biometricContext, Supplier supplier2, boolean z, Map map, boolean z2) {
    ...
    public void startHalOperation() {
        ...
        try {
            if (Utils.DEBUG) {
                Slog.d("FingerprintUpdateActiveUserClient", "startHalOperation: setActiveGroup, " + getTargetUserId());
            }
            int targetUserId = getTargetUserId();
            Slog.d("FingerprintUpdateActiveUserClient", "Setting active user: " + targetUserId);
            ((IBiometricsFingerprint) getFreshDaemon()).setActiveGroup(targetUserId, this.mDirectory.getAbsolutePath());
