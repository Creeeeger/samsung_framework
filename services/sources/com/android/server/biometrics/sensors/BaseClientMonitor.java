...
public abstract class BaseClientMonitor implements IBinder.DeathRecipient {
    ...
    public void binderDiedInternal(final boolean z) {
        getMainHandlerWithModality().post(new Runnable() { // from class: com.android.server.biometrics.sensors.BaseClientMonitor$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                BaseClientMonitor.this.lambda$binderDiedInternal$0(z);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$binderDiedInternal$0(boolean z) {
    ...
    public final Handler getMainHandlerWithModality() {
        int statsModality = this.mLogger.getStatsModality();
        if (statsModality == 1) {
            return SemFpMainThread.get().getHandler();
        }
        if (statsModality == 4) {
            return SemFaceMainThread.get().getHandler();
        }
        return getContext().getMainThreadHandler();
    }
}
