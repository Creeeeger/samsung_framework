...
public class GestureLauncherService extends SystemService {
    ...
    public void onBootPhase(int i) {
        if (i == 600) {
            if (isGestureLauncherEnabled(this.mContext.getResources())) {
                ...
                this.mVibrateMilliSecondsForPanicGesture = r5.getInteger(R.integer.config_veryLongPressOnPowerBehavior);
            }
        }
    }
    ...
    public final void registerCameraLaunchGesture(Resources resources) {
        ...
        int integer = resources.getInteger(R.integer.config_displayWhiteBalanceDecreaseDebounce);
        if (integer != -1) {
            this.mCameraLaunchRegistered = false;
            String string = resources.getString(R.string.eventTypeCustom);
    ...
    public final void registerCameraLiftTrigger(Resources resources) {
        ...
        SensorManager sensorManager = (SensorManager) this.mContext.getSystemService("sensor");
        int integer = resources.getInteger(R.integer.config_displayWhiteBalanceIncreaseDebounce);
        if (integer != -1) {
            this.mCameraLiftRegistered = false;
            String string = resources.getString(R.string.eventTypeOther);
            Sensor defaultSensor = sensorManager.getDefaultSensor(integer, true);
    ...
    public static boolean isCameraLaunchEnabled(Resources resources) {
        return (resources.getInteger(R.integer.config_displayWhiteBalanceDecreaseDebounce) != -1) && !SystemProperties.getBoolean("gesture.disable_camera_launch", false);
    }

    public static boolean isCameraDoubleTapPowerEnabled(Resources resources) {
        return resources.getBoolean(R.bool.config_device_wfc_ims_available);
    }

    public static boolean isCameraLiftTriggerEnabled(Resources resources) {
        return resources.getInteger(R.integer.config_displayWhiteBalanceIncreaseDebounce) != -1;
    }

    public static boolean isEmergencyGestureEnabled(Resources resources) {
        return resources.getBoolean(17891653);
    }

    public static boolean isDefaultEmergencyGestureEnabled(Resources resources) {
        return resources.getBoolean(R.bool.kg_share_status_area);
    }
    ...
    public boolean interceptPowerKeyDown(KeyEvent keyEvent, boolean z, MutableBoolean mutableBoolean) {
            ...
            if (this.mEmergencyGestureEnabled) {
                ...
                if (i2 == 5) {
                    ...
                    if (eventTime3 <= Settings.Global.getInt(this.mContext.getContentResolver(), "emergency_gesture_tap_detection_min_time_ms", 200)) {
                        Slog.i("GestureLauncherService", "Emergency gesture detected but it's too fast. Gesture time: " + eventTime3 + " ms");
                        this.mFirstPowerDown = keyEvent.getEventTime();
                        this.mPowerButtonConsecutiveTaps = 1;
                        this.mPowerButtonSlowConsecutiveTaps = 1;
                    } else {
                        Slog.i("GestureLauncherService", "Emergency gesture detected. Gesture time: " + eventTime3 + " ms");
        ...
        if (z4) {
            ...
            if (z4) {
                this.mMetricsLogger.action(IDnsResolverUnsolicitedEventListener.DNS_HEALTH_RESULT_TIMEOUT, (int) eventTime);
