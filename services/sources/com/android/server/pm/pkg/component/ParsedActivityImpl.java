...
public class ParsedActivityImpl extends ParsedMainComponentImpl implements ParsedActivity {
    public static final Parcelable.Creator CREATOR = new Parcelable.Creator() { // from class: com.android.server.pm.pkg.component.ParsedActivityImpl.1
    ...
    public ParsedActivityImpl setMaxAspectRatio(int i, float f) {
        if (i == 2 || i == 1 || (f < 1.0f && f != DisplayPowerController2.RATE_FROM_DOZE_TO_ON)) {
            return this;
        }
    ...
    public Set getKnownActivityEmbeddingCerts() {
        Set set = this.mKnownActivityEmbeddingCerts;
        return set == null ? Collections.emptySet() : set;
    }

    public void setKnownActivityEmbeddingCerts(Set set) {
        this.mKnownActivityEmbeddingCerts = new ArraySet();
        Iterator it = set.iterator();
        while (it.hasNext()) {
            this.mKnownActivityEmbeddingCerts.add(((String) it.next()).toUpperCase(Locale.US));
        }
    }
