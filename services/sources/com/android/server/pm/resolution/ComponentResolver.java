...
public class ComponentResolver extends ComponentResolverLocked implements Snappable {
    ...
    public final void addActivitiesLocked(Computer computer, AndroidPackage androidPackage, List list, boolean z) {
        int size = ArrayUtils.size(androidPackage.getActivities());
        for (int i = 0; i < size; i++) {
            this.mActivities.addActivity(computer, (ParsedActivity) androidPackage.getActivities().get(i), "activity", list);
        }
    }
                ...
                for (int i2 = 0; i2 < split.length; i2++) {
                    ...
                    if (!this.mProvidersByAuthority.containsKey(split[i2])) {
                        ...
                        if (parsedProvider.getAuthority() == null) {
                            ...
                            ComponentMutateUtils.setAuthority(parsedProvider, parsedProvider.getAuthority() + KnoxVpnFirewallHelper.DELIMITER + split[i2]);
                        }
                    } else {
    ...
    public final void addReceiversLocked(Computer computer, AndroidPackage androidPackage, boolean z) {
        ...
        for (int i = 0; i < size; i++) {
            this.mReceivers.addActivity(computer, (ParsedActivity) androidPackage.getReceivers().get(i), "receiver", null);
        }
    }
    ...
    public static ParsedActivity findMatchingActivity(List list, ParsedActivity parsedActivity) {
        ...
        while (it.hasNext()) {
            ...
            if (parsedActivity2.getName().equals(parsedActivity.getName()) || parsedActivity2.getName().equals(parsedActivity.getTargetActivity())) {
                return parsedActivity2;
            }
            if (parsedActivity2.getTargetActivity() != null && (parsedActivity2.getTargetActivity().equals(parsedActivity.getName()) || parsedActivity2.getTargetActivity().equals(parsedActivity.getTargetActivity()))) {
                return parsedActivity2;
            }
        }
        return null;
    ...
    public final void adjustPriority(Computer computer, List list, ParsedActivity parsedActivity, ParsedIntentInfo parsedIntentInfo, String str) {
    ...
    public final void removeAllComponentsLocked(AndroidPackage androidPackage, boolean z) {
        int size = ArrayUtils.size(androidPackage.getActivities());
        for (int i = 0; i < size; i++) {
            this.mActivities.removeActivity((ParsedActivity) androidPackage.getActivities().get(i), "activity");
        }
        ...
        for (int i4 = 0; i4 < size3; i4++) {
            this.mReceivers.removeActivity((ParsedActivity) androidPackage.getReceivers().get(i4), "receiver");
        }
    ...
    public abstract class MimeGroupsAwareIntentResolver extends IntentResolver {
        ...
        public void addFilter(PackageDataSnapshot packageDataSnapshot, Pair pair) {
            ...
            register_intent_filter(pair, intentFilter.mimeGroupsIterator(), this.mMimeGroupToFilter, "      MimeGroup: ");
        }
        ...
        public void removeFilterInternal(Pair pair) {
            IntentFilter intentFilter = getIntentFilter(pair);
            if (!this.mIsUpdatingMimeGroup) {
                unregister_intent_filter(pair, intentFilter.mimeGroupsIterator(), this.mMimeGroupToFilter, "      MimeGroup: ");
            }
        ...
        public final void applyMimeGroups(Computer computer, Pair pair) {
            ...
            for (int countMimeGroups = intentFilter.countMimeGroups() - 1; countMimeGroups >= 0; countMimeGroups--) {
                PackageStateInternal packageStateInternal = computer.getPackageStateInternal(((ParsedComponent) pair.first).getPackageName());
                if (packageStateInternal == null) {
                    collection = Collections.emptyList();
                } else {
                    collection = (Collection) packageStateInternal.getMimeGroups().get(intentFilter.getMimeGroup(countMimeGroups));
                }
    ...
    public class ActivityIntentResolver extends MimeGroupsAwareIntentResolver {
        ...
        public /* bridge */ /* synthetic */ void addFilter(PackageDataSnapshot packageDataSnapshot, Pair pair) {
        ...
        public List queryIntentForPackage(Computer computer, Intent intent, String str, long j, List list, int i) {
            if (!this.mUserManager.exists(i)) {
                return null;
            }
            if (list == null) {
        ...
        public void addActivity(Computer computer, ParsedActivity parsedActivity, String str, List list) {
    ...
    public final class InstantAppIntentResolver extends IntentResolver {
        ...
        public void filterResults(List list) {
            ...
            int i = 0;
            while (i < size) {
                InstantAppResolveInfo instantAppResolveInfo = ((AuxiliaryResolveInfo.AuxiliaryFilter) list.get(i)).resolveInfo;
