...
public final class PackageSessionVerifier {
    ...
    public PackageSessionVerifier(Context context, PackageManagerService packageManagerService, ApexManager apexManager, Supplier supplier, Looper looper) {
    ...
    public final SigningDetails getSigningDetails(PackageInfo packageInfo) {
    ...
    public final void verifyAPK(final PackageInstallerSession packageInstallerSession, final Callback callback) {
    ...
    public /* synthetic */ void lambda$verifyStaged$1(StagingManager.StagedSession stagedSession, Callback callback) {
        try {
            ...
            if (stagedSession.isMultiPackage()) {
                Iterator it = stagedSession.getChildSessions().iterator();
                while (it.hasNext()) {
                    checkOverlaps(stagedSession, (StagingManager.StagedSession) it.next());
                }
            } else {
    ...
    public final void submitSessionToApexService(StagingManager.StagedSession stagedSession, int i) {
        String string;
        boolean z;
                ...
                try {
                    ...
                    try {
                        ...
                        if (metaData != null && (string = metaData.getString("com.samsung.android.support.targets")) != null) {
                            ...
                            if (!z) {
                                throw new PackageManagerException(-22, "Supported targets(" + string + ") are specified but no matched one with " + str);
                            }
                        }
    ...
    public final int retrieveRollbackIdForCommitSession(int i) {
    ...
    public void checkRebootlessApex(PackageInstallerSession packageInstallerSession) {
    ...
    public final void checkActiveSessions() {
        try {
    ...
    public void checkActiveSessions(boolean z) {
        int i = 0;
        for (StagingManager.StagedSession stagedSession : this.mStagedSessions) {
    ...
    public void checkRollbacks(StagingManager.StagedSession stagedSession) {
        if (stagedSession.isDestroyed() || stagedSession.isInTerminalState()) {
            return;
        }
    ...
    public void checkOverlaps(StagingManager.StagedSession stagedSession, StagingManager.StagedSession stagedSession2) {
        if (stagedSession.isDestroyed() || stagedSession.isInTerminalState()) {
            return;
        }
