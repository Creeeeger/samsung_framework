...
public class UsbMidiBlockParser {
    public ArrayList mGroupTerminalBlocks = new ArrayList();
    ...
    /* loaded from: classes3.dex */
    public class GroupTerminalBlock {
    ...
    public int calculateMidiType(UsbDeviceConnection usbDeviceConnection, int i, int i2) {
        ...
        try {
            ...
            if (controlTransfer <= 0) {
                ...
                int i4 = (bArr[3] & IDnsResolverUnsolicitedEventListener.DNS_HEALTH_RESULT_TIMEOUT) + ((bArr[4] & IDnsResolverUnsolicitedEventListener.DNS_HEALTH_RESULT_TIMEOUT) << 8);
                if (i4 <= 0) {
                ...
                if (controlTransfer2 > 0) {
                    ...
                    Log.d("UsbMidiBlockParser", "MIDI protocol: " + ((GroupTerminalBlock) this.mGroupTerminalBlocks.get(0)).mMidiProtocol);
                    return ((GroupTerminalBlock) this.mGroupTerminalBlocks.get(0)).mMidiProtocol;
                }
    ...
    public void dump(DualDumpOutputStream dualDumpOutputStream, String str, long j) {
        ...
        Iterator it = this.mGroupTerminalBlocks.iterator();
        while (it.hasNext()) {
            ((GroupTerminalBlock) it.next()).dump(dualDumpOutputStream, "block", 2246267895813L);
        }
        dualDumpOutputStream.end(start);
    }
}
