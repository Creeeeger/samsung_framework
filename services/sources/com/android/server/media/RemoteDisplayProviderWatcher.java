...
public final class RemoteDisplayProviderWatcher {
    ...
    public void stop() {
        if (this.mRunning) {
            ...
            for (int size = this.mProviders.size() - 1; size >= 0; size--) {
                ((RemoteDisplayProviderProxy) this.mProviders.get(size)).stop();
            }
        }
    }
        ...
        if (this.mRunning) {
            ...
            while (it.hasNext()) {
                ...
                if (serviceInfo != null && verifyServiceTrusted(serviceInfo)) {
                    ...
                    if (findProvider < 0) {
                        ...
                        RemoteDisplayProviderProxy remoteDisplayProviderProxy2 = (RemoteDisplayProviderProxy) this.mProviders.get(findProvider);
            ...
            if (i2 < this.mProviders.size()) {
                for (int size = this.mProviders.size() - 1; size >= i2; size--) {
                    RemoteDisplayProviderProxy remoteDisplayProviderProxy3 = (RemoteDisplayProviderProxy) this.mProviders.get(size);
    ...
    public final int findProvider(String str, String str2) {
        ...
        for (int i = 0; i < size; i++) {
            if (((RemoteDisplayProviderProxy) this.mProviders.get(i)).hasComponentName(str, str2)) {
                return i;
            }
        }
        return -1;
