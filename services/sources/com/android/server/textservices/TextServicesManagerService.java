...
public class TextServicesManagerService extends ITextServicesManager.Stub {
    ...
    public class TextServicesData {
        ...
        public SpellCheckerInfo getCurrentSpellChecker() {
            ...
            return (SpellCheckerInfo) this.mSpellCheckerMap.get(selectedSpellChecker);
        }
        ...
        public final void dump(PrintWriter printWriter) {
            ...
            for (Map.Entry entry : this.mSpellCheckerBindGroups.entrySet()) {
                ...
                printWriter.println("    " + ((String) entry.getKey()) + " " + spellCheckerBindGroup + XmlUtils.STRING_ARRAY_SEPARATOR);
                StringBuilder sb = new StringBuilder();
                sb.append("      mInternalConnection=");
                sb.append(spellCheckerBindGroup.mInternalConnection);
                printWriter.println(sb.toString());
                printWriter.println("      mSpellChecker=" + spellCheckerBindGroup.mSpellChecker);
                printWriter.println("      mUnbindCalled=" + spellCheckerBindGroup.mUnbindCalled);
                printWriter.println("      mConnected=" + spellCheckerBindGroup.mConnected);
                ...
                int registeredCallbackCount = spellCheckerBindGroup.mListeners.getRegisteredCallbackCount();
                for (int i4 = 0; i4 < registeredCallbackCount; i4++) {
                    ISpellCheckerSessionListener registeredCallbackItem = spellCheckerBindGroup.mListeners.getRegisteredCallbackItem(i4);
    ...
    public final SpellCheckerInfo findAvailSystemSpellCheckerLocked(String str, TextServicesData textServicesData) {
        ...
        ArrayList suitableLocalesForSpellChecker = LocaleUtils.getSuitableLocalesForSpellChecker(this.mContext.getResources().getConfiguration().locale);
        ...
        if (DumpUtils.checkDumpPermission(this.mContext, TAG, printWriter)) {
            if (strArr.length == 0 || (strArr.length == 1 && strArr[0].equals("-a"))) {
                synchronized (this.mLock) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ((TextServicesData) this.mUserData.valueAt(i)).dump(printWriter);
                    }
                }
                return;
            }
    ...
    public final class SpellCheckerBindGroup {
        ...
        public HashMap mSpellCheckerBindGroups;
        ...
        public void onServiceConnectedLocked(ISpellCheckerService iSpellCheckerService) {
            ...
            try {
                ...
                for (int i = 0; i < size; i++) {
                    SessionRequest sessionRequest = (SessionRequest) this.mPendingSessionRequests.get(i);
