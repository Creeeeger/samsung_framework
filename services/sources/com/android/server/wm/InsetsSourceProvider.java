...
public class InsetsSourceProvider {
    ...
    public final Consumer mSetLeashPositionConsumer = new Consumer() { // from class: com.android.server.wm.InsetsSourceProvider$$ExternalSyntheticLambda0
    ...
    public /* synthetic */ void lambda$new$0(SurfaceControl.Transaction transaction) {
        ...
        Slog.d("InsetsSourceProvider", "Update insets leash position=" + surfacePosition + ", leash=" + leash + ", caller=" + Debug.getCallers(3));
    }
    ...
    public void onPostLayout() {
        ...
        if (this.mControl != null) {
            ...
            if ((i == 2019 || i == 2024) && !this.mControl.getSurfacePosition().equals(windowFrameSurfacePosition)) {
                Slog.d("InsetsSourceProvider", "onPostLayout, current position=" + this.mControl.getSurfacePosition() + " ->  position=" + windowFrameSurfacePosition + ", controlTarget=" + this.mControlTarget + ", insetsWin=" + asWindowState);
            }
            if (this.mControl.setSurfacePosition(windowFrameSurfacePosition.x, windowFrameSurfacePosition.y) && this.mControlTarget != null) {
                if (asWindowState.getWindowFrames().didFrameSizeChange() && asWindowState.mWinAnimator.getShown() && this.mWindowContainer.okToDisplay()) {
                    if (asWindowState.mAttrs.type == 2024 && asWindowState.getWindowFrames().didFrameHeightChange()) {
                        Slog.d("InsetsSourceProvider", "onPostLayout, skip ensuring control position for " + this.mControl);
                    ...
                    if (asyncRotationController != null && (drawTransaction = asyncRotationController.getDrawTransaction(asWindowState.mToken)) != null) {
    ...
    public void updateFakeControlTarget(InsetsControlTarget insetsControlTarget) {
        ...
        Slog.d("InsetsSourceProvider", "updateFakeControlTarget: fakeControl=" + this.mFakeControl + ", fakeTarget=" + insetsControlTarget);
    }
    ...
    public void updateControlForTarget(InsetsControlTarget insetsControlTarget, boolean z) {
        ...
        if (insetsControlTarget != this.mControlTarget || z) {
            if (insetsControlTarget == null) {
                if (this.mDisplayContent.getDefaultTaskDisplayArea().isSplitScreenVisible() && this.mWindowContainer.getWindow() != null) {
            ...
            if ((this.mSource.getType() == WindowInsets.Type.navigationBars() && insetsControlTarget.getWindow() != null && insetsControlTarget.getWindow().mToken.isFixedRotationTransforming()) || this.mWindowContainer.getSyncTransaction() == this.mWindowContainer.mSyncTransaction) {
                return;
            }
            Point windowFrameSurfacePosition = getWindowFrameSurfacePosition();
            if (this.mDisplayContent.getDisplayId() == 2 && this.mWindowContainer.asWindowState() != null && this.mWindowContainer.asWindowState().getWindowFrames().mFrame.isEmpty() && this.mWindowContainer.asWindowState().getWindowType() == 2019 && windowFrameSurfacePosition.y != (i = this.mDisplayContent.getBounds().bottom - this.mDisplayContent.getDisplayPolicy().mDexTaskbarHeight)) {
                windowFrameSurfacePosition.y = i;
                Slog.d("InsetsSourceProvider", "updateControlForTarget: change surfacePos=" + windowFrameSurfacePosition);
            }
            this.mAdapter = new ControlAdapter(windowFrameSurfacePosition);
            if (this.mSource.getType() == WindowInsets.Type.ime()) {
                DisplayContent displayContent = this.mDisplayContent;
                if (displayContent.mRemoteInsetsControlTarget == this.mControlTarget && displayContent.getImeInputTarget() == insetsControlTarget && this.mClientVisible) {
    ...
    public boolean shouldDeferUpdate() {
        ...
        if (windowContainer != null && this.mControl != null) {
            WindowState asWindowState = windowContainer.asWindowState();
            Point windowFrameSurfacePosition = getWindowFrameSurfacePosition();
            if (asWindowState != null && asWindowState.mAttrs.type == 2024 && asWindowState.mInRelayout && !this.mControl.getSurfacePosition().equals(windowFrameSurfacePosition)) {
