...
public class RecentsAnimationController implements IBinder.DeathRecipient {
    ...
    public final IRecentsAnimationController mController = new IRecentsAnimationController.Stub() { // from class: com.android.server.wm.RecentsAnimationController.2
        public TaskSnapshot screenshotTask(int i) {
            if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 184362060, 13, "screenshotTask(%d): mCanceled=%b", new Object[]{Long.valueOf(i), Boolean.valueOf(RecentsAnimationController.this.mCanceled)});
            }
        ...
        public void setFinishTaskTransaction(int i, PictureInPictureSurfaceTransaction pictureInPictureSurfaceTransaction, SurfaceControl surfaceControl) {
            if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -163974242, 1, "setFinishTaskTransaction(%d): transaction=%s", new Object[]{Long.valueOf(i), String.valueOf(pictureInPictureSurfaceTransaction)});
            }
            long clearCallingIdentity = Binder.clearCallingIdentity();
            try {
                synchronized (RecentsAnimationController.this.mService.getWindowManagerLock()) {
        ...
        public void finish(boolean z, boolean z2) {
            if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -445944810, 15, "finish(%b): mCanceled=%b", new Object[]{Boolean.valueOf(z), Boolean.valueOf(RecentsAnimationController.this.mCanceled)});
            }
            long clearCallingIdentity = Binder.clearCallingIdentity();
            try {
        ...
        public void setInputConsumerEnabled(boolean z) {
            if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -1219773477, 12, "setInputConsumerEnabled(%s): mCanceled=%b", new Object[]{String.valueOf(z), Boolean.valueOf(RecentsAnimationController.this.mCanceled)});
            }
            long clearCallingIdentity = Binder.clearCallingIdentity();
            try {
                synchronized (RecentsAnimationController.this.mService.getWindowManagerLock()) {
    ...
    public void initialize(int i, SparseBooleanArray sparseBooleanArray, ActivityRecord activityRecord) {
        ...
        final ArrayList visibleTasks = this.mDisplayContent.getDefaultTaskDisplayArea().getVisibleTasks();
        ...
        for (int size = visibleTasks.size() - 1; size >= 0; size--) {
            final Task task = (Task) visibleTasks.get(size);
            if (!skipAnimation(task)) {
                if (task.mIsDragSourceTask) {
                    Slog.d(TAG, "[TWODND] initialize-DragSourceTask");
        ...
        try {
            ...
            if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1519757176, 0, "setHomeApp(%s)", new Object[]{String.valueOf(activityRecord.getName())});
            }
            ...
            this.mService.mAtmService.mMultiTaskingController.notifyStartRecentsAnimation(true);
    ...
    public static /* synthetic */ void lambda$initialize$2(Task task, final int i, final AnimationAdapter animationAdapter) {
        task.forAllWindows(new Consumer() { // from class: com.android.server.wm.RecentsAnimationController$$ExternalSyntheticLambda3
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
    ...
    public boolean isInterestingForAllDrawn(WindowState windowState) {
        return (isTargetApp(windowState.getActivityRecord()) && windowState.getWindowType() != 1 && windowState.getAttrs().alpha == DisplayPowerController2.RATE_FROM_DOZE_TO_ON) ? false : true;
    }
    ...
    public TaskAnimationAdapter addAnimation(Task task, boolean z, boolean z2, SurfaceAnimator.OnAnimationFinishedCallback onAnimationFinishedCallback) {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 302992539, 0, "addAnimation(%s)", new Object[]{String.valueOf(task.getName())});
        }
    ...
    public void removeAnimation(TaskAnimationAdapter taskAnimationAdapter) {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 83950285, 1, "removeAnimation(%d)", new Object[]{Long.valueOf(taskAnimationAdapter.mTask.mTaskId)});
        }
    ...
    public void removeWallpaperAnimation(WallpaperAnimationAdapter wallpaperAnimationAdapter) {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -1768557332, 0, "removeWallpaperAnimation()", (Object[]) null);
        }
    ...
    public void startAnimation() {
        ...
        Rect rect;
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 17696244, 15, "startAnimation(): mPendingStart=%b mCanceled=%b", new Object[]{Boolean.valueOf(this.mPendingStart), Boolean.valueOf(this.mCanceled)});
        }
        ...
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -1207757583, 0, "startAnimation(): Notify animation start: %s", new Object[]{String.valueOf(this.mPendingAnimations.stream().map(new Function() { // from class: com.android.server.wm.RecentsAnimationController$$ExternalSyntheticLambda5
    ...
    public final void attachNavigationBarToApp() {
        ...
        if (this.mDisplayContent.getDisplayPolicy().shouldAttachNavBarToAppDuringTransition() && this.mDisplayContent.getAsyncRotationController() == null) {
            ...
            while (true) {
                ...
                Task task = ((TaskAnimationAdapter) this.mPendingAnimations.get(size)).mTask;
                if (!task.isActivityTypeHomeOrRecents()) {
                    this.mNavBarAttachedApp = task.getTopVisibleActivity(false, false);
                    break;
                }
                size--;
            }
            ...
            if (this.mNavBarAttachedApp.mCompatRecord.isCompatModeEnabled()) {
                Point lastSurfacePosition = this.mNavBarAttachedApp.getLastSurfacePosition();
                navigationBarWindow.setSurfaceTranslationX(-lastSurfacePosition.x);
                navigationBarWindow.setSurfaceTranslationY(-lastSurfacePosition.y);
            }
    ...
    public void restoreNavigationBarFromApp(boolean z) {
        if (this.mNavigationBarAttachedToApp) {
            ...
            navigationBarWindow.setSurfaceTranslationX(0);
    ...
    public final void collectTaskRemoteAnimations(Task task, final int i, final SurfaceAnimator.OnAnimationFinishedCallback onAnimationFinishedCallback) {
        ...
        task.forAllLeafTasks(new Consumer() { // from class: com.android.server.wm.RecentsAnimationController$$ExternalSyntheticLambda4
            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
    ...
    public /* synthetic */ void lambda$collectTaskRemoteAnimations$4(SparseBooleanArray sparseBooleanArray, SurfaceAnimator.OnAnimationFinishedCallback onAnimationFinishedCallback, int i, Task task) {
        if (task.shouldBeVisible(null)) {
            ...
            if (createRemoteAnimationTarget != null) {
                ...
                if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
                    ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1151072840, 0, "collectTaskRemoteAnimations, target: %s", new Object[]{String.valueOf(createRemoteAnimationTarget)});
                }
            }
        }
    }
    ...
    public final RemoteAnimationTarget[] createWallpaperAnimations() {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1495525537, 0, "createWallpaperAnimations()", (Object[]) null);
        }
    ...
    public final void cancelAnimation(int i, boolean z, String str) {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, 1525976603, 0, "cancelAnimation(): reason=%s", new Object[]{String.valueOf(str)});
        }
    ...
    public void cleanupAnimation(int i) {
        if (ProtoLogCache.WM_DEBUG_RECENTS_ANIMATIONS_enabled) {
            ProtoLogImpl.d(ProtoLogGroup.WM_DEBUG_RECENTS_ANIMATIONS, -1434147454, 5, "cleanupAnimation(): Notify animation finished mPendingAnimations=%d reorderMode=%d", new Object[]{Long.valueOf(this.mPendingAnimations.size()), Long.valueOf(i)});
        }
        ...
        for (int size2 = this.mPendingWallpaperAnimations.size() - 1; size2 >= 0; size2--) {
            removeWallpaperAnimation((WallpaperAnimationAdapter) this.mPendingWallpaperAnimations.get(size2));
        }
        ...
        this.mService.mAtmService.mMultiTaskingController.notifyFinishRecentsAnimation(i != 2);
        StatusBarManagerInternal statusBarManagerInternal = this.mStatusBar;
        if (statusBarManagerInternal != null) {
    ...
    public class TaskAnimationAdapter implements AnimationAdapter {
        ...
        public RemoteAnimationTarget createRemoteAnimationTarget(int i, int i2) {
            ...
            if (topRealVisibleActivity == null) {
                topRealVisibleActivity = this.mTask.getTopVisibleActivity(false, false);
            }
        ...
        public void setSnapshotOverlay(TaskSnapshot taskSnapshot) {
            ...
            this.mSnapshotOverlay = ((SurfaceControl.Builder) RecentsAnimationController.this.mService.mSurfaceControlFactory.apply(new SurfaceSession())).setName("RecentTaskScreenshotSurface").setCallsite("TaskAnimationAdapter.setSnapshotOverlay").setFormat(hardwareBuffer.getFormat()).setParent(this.mCapturedLeash).setBLASTLayer().build();
            float width = (this.mTask.getBounds().width() * 1.0f) / hardwareBuffer.getWidth();
            this.mTask.getPendingTransaction().setBuffer(this.mSnapshotOverlay, GraphicBuffer.createFromHardwareBuffer(hardwareBuffer)).setColorSpace(this.mSnapshotOverlay, taskSnapshot.getColorSpace()).setLayer(this.mSnapshotOverlay, Integer.MAX_VALUE).setMatrix(this.mSnapshotOverlay, width, DisplayPowerController2.RATE_FROM_DOZE_TO_ON, DisplayPowerController2.RATE_FROM_DOZE_TO_ON, width).show(this.mSnapshotOverlay).apply();
        }
