...
public class AlertWindowNotification {
    ...
    public final void onPostNotification() {
        ...
        String string = context.getString(R.string.capability_desc_canTakeScreenshot, charSequence);
        Bundle bundle = new Bundle();
        bundle.putStringArray("android.foregroundApps", new String[]{this.mPackageName});
        Notification.Builder contentIntent = new Notification.Builder(context, this.mNotificationTag).setOngoing(true).setContentTitle(context.getString(R.string.capability_title_canCaptureFingerprintGestures, charSequence)).setContentText(string).setSmallIcon(R.drawable.autofill_dataset_picker_background).setColor(context.getColor(R.color.system_notification_accent_color)).setStyle(new Notification.BigTextStyle().bigText(string)).setLocalOnly(true).addExtras(bundle).setContentIntent(getContentIntent(context, this.mPackageName));
    ...
    public final void createNotificationChannel(Context context, String str) {
        if (sChannelGroup == null) {
            NotificationChannelGroup notificationChannelGroup = new NotificationChannelGroup("com.android.server.wm.AlertWindowNotification - ", this.mService.mContext.getString(R.string.capability_desc_canRequestTouchExploration));
            sChannelGroup = notificationChannelGroup;
            this.mNotificationManager.createNotificationChannelGroup(notificationChannelGroup);
        }
        String string = context.getString(R.string.capability_desc_canRetrieveWindowContent, str);
