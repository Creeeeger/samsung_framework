...
public class Session extends IWindowSession.Stub implements IBinder.DeathRecipient {
    ...
    public IBinder performDragWithArea(IWindow iWindow, int i, SurfaceControl surfaceControl, int i2, float f, float f2, float f3, float f4, ClipData clipData, RectF rectF, Point point) {
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
            return this.mDragDropController.performDragWithArea(this.mPid, this.mUid, iWindow, i, surfaceControl, i2, f, f2, f3, f4, clipData, rectF, point);
        } finally {
            Binder.restoreCallingIdentity(clearCallingIdentity);
        }
    }
    ...
    public void validateAndResolveDragMimeTypeExtras(ClipData clipData, int i, int i2, String str) {
        ...
        int i6 = (this.mService.mAtmService.mMultiTaskingController.getSystemUiUid() == i || this.mService.mAtmService.getRecentTasks().isRecentsUid(i) || this.mService.mAtmService.getRecentTasks().isLauncherUid(i)) ? 1000 : i;
        if (hasMimeType) {
            long clearCallingIdentity = Binder.clearCallingIdentity();
            for (int i7 = 0; i7 < clipData.getItemCount(); i7++) {
                try {
                    ...
                    itemAt.setActivityInfo(this.mService.mAtmService.resolveActivityInfoForIntent(this.mService.mAmInternal.getIntentForIntentSender(pendingIntent.getIntentSender().getTarget()), null, userHandle.getIdentifier(), i6, i2));
                } finally {
                    Binder.restoreCallingIdentity(clearCallingIdentity);
                }
            }
            ...
            if (hasMimeType3) {
                ...
                for (int i9 = 0; i9 < clipData.getItemCount(); i9++) {
                    ...
                    if (anyTaskForId.getRootActivity() != null) {
                        ...
                        itemAt2.setActivityInfo(this.mService.mAtmService.resolveActivityInfoForIntent(anyTaskForId.intent, null, anyTaskForId.mUserId, i8, i2));
                    }
                }
                return;
            }
        ...
        while (i10 < clipData.getItemCount()) {
            ...
            if (i11 == 1000) {
                item = itemAt3;
                i3 = i10;
                resolveActivityInfoForIntent = this.mService.mAtmService.resolveActivityInfoForIntent(createShortcutIntents[0], createShortcutIntents[0].resolveTypeIfNeeded(this.mService.mContext.getContentResolver()), userHandle2.getIdentifier(), i11, i2);
            } else {
                item = itemAt3;
                i3 = i10;
                resolveActivityInfoForIntent = this.mService.mAtmService.resolveActivityInfoForIntent(createShortcutIntents[0], null, userHandle2.getIdentifier(), i, i2);
            }
    ...
    public void setWallpaperZoomOut(IBinder iBinder, final float f) {
        if (Float.compare(DisplayPowerController2.RATE_FROM_DOZE_TO_ON, f) > 0 || Float.compare(1.0f, f) < 0 || Float.isNaN(f)) {
    ...
    public void onWindowSurfaceVisibilityChanged(WindowSurfaceController windowSurfaceController, boolean z, int i, boolean z2) {
        boolean remove;
        boolean remove2;
        if (z != z2) {
            if (z) {
    ...
    public boolean hasAlertWindowSurfaces(DisplayContent displayContent) {
        for (int size = this.mAlertWindowSurfaces.size() - 1; size >= 0; size--) {
            if (((WindowSurfaceController) this.mAlertWindowSurfaces.valueAt(size)).mAnimator.mWin.getDisplayContent() == displayContent) {
                return true;
            }
        }
        return false;
    ...
    public void grantInputChannelWithTaskToken(int i, SurfaceControl surfaceControl, IWindow iWindow, IBinder iBinder, int i2, int i3, int i4, int i5, IBinder iBinder2, IBinder iBinder3, String str, InputChannel inputChannel, int i6, WindowContainerToken windowContainerToken) {
        if (iBinder == null && !this.mCanAddInternalSystemWindow) {
            throw new SecurityException("Requires INTERNAL_SYSTEM_WINDOW permission");
        }
        long clearCallingIdentity = Binder.clearCallingIdentity();
        try {
            this.mService.grantInputChannelWithTaskToken(this, this.mUid, this.mPid, i, surfaceControl, iWindow, iBinder, i2, this.mCanAddInternalSystemWindow ? i3 : 0, i4, i5, iBinder2, iBinder3, str, inputChannel, i6, windowContainerToken);
        } finally {
            Binder.restoreCallingIdentity(clearCallingIdentity);
        }
    }
    ...
    public void setKeyguardWallpaperTouchAllowed(IWindow iWindow, boolean z) {
        ...
        synchronized (windowManagerGlobalLock) {
            try {
                this.mService.windowForClientLocked(this, iWindow, true).setKeyguardWallpaperTouchAllowed(z);
            } catch (Throwable th) {
                WindowManagerService.resetPriorityAfterLockedSection();
                throw th;
            }
