...
public abstract class KeySyncUtils {
    ...
    public static byte[] thmEncryptRecoveryKey(PublicKey publicKey, byte[] bArr, byte[] bArr2, SecretKey secretKey) {
    ...
    public static byte[] calculateThmKfHash(byte[] bArr) {
    ...
    public static byte[] locallyEncryptRecoveryKey(byte[] bArr, SecretKey secretKey) {
    ...
    public static byte[] encryptRecoveryClaim(PublicKey publicKey, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
    ...
    public static byte[] decryptRecoveryClaimResponse(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        return SecureBox.decrypt(null, bArr, ArrayUtils.concat(new byte[][]{RECOVERY_RESPONSE_HEADER, bArr2}), bArr3);
    }

    public static byte[] decryptRecoveryKey(byte[] bArr, byte[] bArr2) {
        return SecureBox.decrypt(null, bArr, LOCALLY_ENCRYPTED_RECOVERY_KEY_HEADER, bArr2);
    }

    public static byte[] decryptApplicationKey(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        byte[] concat;
        if (bArr3 == null) {
            concat = ENCRYPTED_APPLICATION_KEY_HEADER;
        } else {
    ...
    public static PublicKey deserializePublicKey(byte[] bArr) {
        try {
