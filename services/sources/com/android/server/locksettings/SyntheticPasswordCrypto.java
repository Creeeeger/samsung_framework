...
abstract class SyntheticPasswordCrypto {
    ...
    public static byte[] personalizedHashSDP(byte[] bArr, byte[]... bArr2) {
        try {
            MessageDigest messageDigest = MessageDigest.getInstance("SHA-256");
            if (bArr.length > 64) {
                throw new IllegalStateException("Personalization too long");
            }
            messageDigest.update(Arrays.copyOf(bArr, 64));
            for (byte[] bArr3 : bArr2) {
                messageDigest.update(bArr3);
            }
            return messageDigest.digest();
        } catch (NoSuchAlgorithmException e) {
            throw new IllegalStateException("NoSuchAlgorithmException for SHA-256", e);
        }
    }
}
