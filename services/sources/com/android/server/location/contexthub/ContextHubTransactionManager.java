...
public class ContextHubTransactionManager {
    ...
    public synchronized void addTransaction(ContextHubServiceTransaction contextHubServiceTransaction) {
    ...
    public synchronized void onTransactionResponse(int i, boolean z) {
        ...
        if (contextHubServiceTransaction.getTransactionId() != i) {
            Log.w("ContextHubTransactionManager", "Received unexpected transaction response (expected ID = " + contextHubServiceTransaction.getTransactionId() + ", received ID = " + i + ")");
            return;
        }
    ...
    public synchronized void onQueryResponse(List list) {
        ContextHubServiceTransaction contextHubServiceTransaction = (ContextHubServiceTransaction) this.mTransactionQueue.peek();
        if (contextHubServiceTransaction == null) {
            Log.w("ContextHubTransactionManager", "Received unexpected query response (no transaction pending)");
            return;
        }
        if (contextHubServiceTransaction.getTransactionType() != 4) {
            ...
            if (onTransact == 0) {
                Runnable runnable = new Runnable() { // from class: com.android.server.location.contexthub.ContextHubTransactionManager$$ExternalSyntheticLambda0
                    ...
                    public final void run() {
                        ContextHubTransactionManager.this.lambda$startNextTransaction$0(contextHubServiceTransaction);
                    }
                };
                TimeUnit timeUnit = TimeUnit.SECONDS;
                try {
                    this.mTimeoutFuture = this.mTimeoutExecutor.schedule(runnable, contextHubServiceTransaction.getTimeout(timeUnit), timeUnit);
                } catch (Exception e) {
    ...
    public String toString() {
        ...
        for (i = 0; i < contextHubServiceTransactionArr.length; i++) {
            sb.append(i + ": " + contextHubServiceTransactionArr[i] + KnoxVpnFirewallHelper.DELIMITER_IP_RESTORE);
        }
        sb.append("Transaction History:\n");
        Iterator descendingIterator = this.mTransactionRecordDeque.descendingIterator();
        while (descendingIterator.hasNext()) {
            sb.append(descendingIterator.next() + KnoxVpnFirewallHelper.DELIMITER_IP_RESTORE);
        }
        return sb.toString();
    }
}
