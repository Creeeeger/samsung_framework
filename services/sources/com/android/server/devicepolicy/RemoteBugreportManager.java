...
public class RemoteBugreportManager {
    ...
    public final Notification buildNotification(int i) {
        ...
        Notification.Builder extend = new Notification.Builder(this.mContext, SystemNotificationChannels.DEVICE_ADMIN).setSmallIcon(17304219).setOngoing(true).setLocalOnly(true).setContentIntent(PendingIntent.getActivityAsUser(this.mContext, i, intent, 67108864, null, UserHandle.CURRENT)).setColor(this.mContext.getColor(R.color.system_notification_accent_color)).extend(new Notification.TvExtender());
        if (i == 2) {
            extend.setContentTitle(this.mContext.getString(17042811)).setProgress(0, 0, true);
        } else if (i == 1) {
            extend.setContentTitle(this.mContext.getString(17042988)).setProgress(0, 0, true);
        } else if (i == 3) {
            ...
            extend.addAction(new Notification.Action.Builder((Icon) null, this.mContext.getString(R.string.kg_login_account_recovery_hint), PendingIntent.getBroadcast(this.mContext, 678432343, new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_DECLINED"), 335544320)).build()).addAction(new Notification.Action.Builder((Icon) null, this.mContext.getString(17042805), broadcast).build()).setContentTitle(this.mContext.getString(17042807)).setContentText(this.mContext.getString(17042806)).setStyle(new Notification.BigTextStyle().bigText(this.mContext.getString(17042806)));
        }
        return extend.build();
    }
    ...
    public final void onBugreportSharingAccepted() {
        ...
        Pair deviceOwnerRemoteBugreportUriAndHash = this.mService.getDeviceOwnerRemoteBugreportUriAndHash();
