...
public final class NotificationRecord {
    ...
    public float mRankingScore = DisplayPowerController2.RATE_FROM_DOZE_TO_ON;
    ...
    public final Uri calculateSound() {
        ...
        if (this.mContext.getPackageManager().hasSystemFeature("android.software.leanback") || notificationSoundBlockedByMDM()) {
            return null;
        }
    ...
    public final boolean notificationSoundBlockedByMDM() {
        EnterpriseDeviceManager enterpriseDeviceManager;
        ApplicationPolicy applicationPolicy;
        if (this.sbn.getPackageName() != null && (enterpriseDeviceManager = EnterpriseDeviceManager.getInstance(this.mContext)) != null && (applicationPolicy = enterpriseDeviceManager.getApplicationPolicy()) != null) {
            long clearCallingIdentity = Binder.clearCallingIdentity();
            try {
                if (!applicationPolicy.isStatusBarNotificationAllowedAsUser(this.sbn.getPackageName(), UserHandle.getUserId(this.sbn.getUid()))) {
                    if (applicationPolicy.getApplicationNotificationModeAsUser(UserHandle.getUserId(this.sbn.getUid())) == 4) {
    ...
    public final Light calculateLights() {
        int color = this.mContext.getResources().getColor(R.color.resolver_empty_state_icon);
        int integer = this.mContext.getResources().getInteger(R.integer.config_lowBatteryCloseWarningBump);
        int integer2 = this.mContext.getResources().getInteger(R.integer.config_lowBatteryAutoTriggerDefaultLevel);
    ...
    public void dump(PrintWriter printWriter, String str, Context context, boolean z) {
        ...
        printWriter.println(str2 + "mRankingTimeMs=" + this.mRankingTimeMs + "(" + simpleDateFormat.format(new Date(this.mRankingTimeMs)) + ")");
        printWriter.println(str2 + "mCreationTimeMs=" + this.mCreationTimeMs + "(" + simpleDateFormat.format(new Date(this.mCreationTimeMs)) + ")");
        printWriter.println(str2 + "mVisibleSinceMs=" + this.mVisibleSinceMs + "(" + simpleDateFormat.format(new Date(this.mVisibleSinceMs)) + ")");
        printWriter.println(str2 + "mUpdateTimeMs=" + this.mUpdateTimeMs + "(" + simpleDateFormat.format(new Date(this.mUpdateTimeMs)) + ")");
        printWriter.println(str2 + "mInterruptionTimeMs=" + this.mInterruptionTimeMs + "(" + simpleDateFormat.format(new Date(this.mInterruptionTimeMs)) + ")");
        StringBuilder sb3 = new StringBuilder();
        sb3.append(str2);
        sb3.append("mSuppressedVisualEffects= ");
        sb3.append(this.mSuppressedVisualEffects);
        ...
        sb5.append(str2);
        sb5.append("shortcut=");
        sb5.append(notification.getShortcutId());
        sb5.append(" found valid? ");
        sb5.append(this.mShortcutInfo != null);
        printWriter.println(sb5.toString());
        printWriter.println(str2 + "mUserVisOverride=" + getPackageVisibilityOverride());
        printWriter.println(str2 + "mSuggestionsGeneratedByAssistant=" + this.mSuggestionsGeneratedByAssistant);
        ArrayList arrayList = this.mSystemGeneratedSmartActions;
        int size = arrayList != null ? arrayList.size() : 0;
        if (size != 0) {
            printWriter.println(str2 + "mSystemGeneratedSmartActions=");
            for (int i = 0; i < size; i++) {
                ...
                objArr[4] = str3;
                printWriter.println(String.format("%s    [%d] \"%s\" \"%s\"-> %s", objArr));
            }
        }
        ArrayList arrayList2 = this.mSmartReplies;
        int size2 = arrayList2 != null ? arrayList2.size() : 0;
        if (size2 != 0) {
            printWriter.println(str2 + "mSmartReplies=");
            for (int i2 = 0; i2 < size2; i2++) {
                printWriter.println(String.format("%s      [%d] %s", str2, Integer.valueOf(i2), ((CharSequence) this.mSmartReplies.get(i2)).toString()));
            }
        }
    }
    ...
    public final void dumpNotification(PrintWriter printWriter, String str, Notification notification, boolean z) {
        ...
        printWriter.println(str + "parcelDataSize=" + notification.parcelDataSize);
        printWriter.println(str + "mAllowSystemGeneratedContextualActions=" + notification.getAllowSystemGeneratedContextualActions());
    ...
    public ArrayList getPeopleOverride() {
    ...
    public void setInterruptive(boolean z) {
        ...
        if (z) {
            MetricsLogger.action(getLogMaker().setCategory(1501).setType(1).addTaggedData(NetworkConstants.ETHER_MTU, Integer.valueOf(getInterruptionMs(currentTimeMillis))));
    ...
    public void setPeopleOverride(ArrayList arrayList) {
        this.mPeopleOverride = arrayList;
    }

    public ArrayList getSnoozeCriteria() {
        return this.mSnoozeCriteria;
    }

    public void setSnoozeCriteria(ArrayList arrayList) {
        this.mSnoozeCriteria = arrayList;
    }

    public boolean getAlertLimited() {
        return this.mIsAlertLimited;
    }

    public void setAlertLimited(boolean z) {
        this.mIsAlertLimited = z;
    }
    ...
    public void setSystemGeneratedSmartActions(ArrayList arrayList) {
        this.mSystemGeneratedSmartActions = arrayList;
    }

    public ArrayList getSystemGeneratedSmartActions() {
        return this.mSystemGeneratedSmartActions;
    }

    public void setSmartReplies(ArrayList arrayList) {
        this.mSmartReplies = arrayList;
    }

    public ArrayList getSmartReplies() {
    ...
    public ArraySet getGrantableUris() {
    ...
    public LogMaker getLogMaker(long j) {
        LogMaker addTaggedData = getSbn().getLogMaker().addTaggedData(858, Integer.valueOf(this.mImportance)).addTaggedData(793, Integer.valueOf(getLifespanMs(j))).addTaggedData(795, Integer.valueOf(getFreshnessMs(j))).addTaggedData(794, Integer.valueOf(getExposureMs(j))).addTaggedData(NetworkConstants.ETHER_MTU, Integer.valueOf(getInterruptionMs(j)));
        int i = this.mImportanceExplanationCode;
        if (i != 0) {
    ...
    public void mergePhoneNumbers(ArraySet arraySet) {
        ...
        this.mPhoneNumbers.addAll(arraySet);
    }
