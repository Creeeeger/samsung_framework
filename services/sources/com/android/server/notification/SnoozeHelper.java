...
public final class SnoozeHelper {
    ...
    public ArrayList getNotifications(String str, String str2, Integer num) {
        ...
        synchronized (this.mLock) {
            for (int i = 0; i < this.mSnoozedNotifications.size(); i++) {
                NotificationRecord notificationRecord = (NotificationRecord) this.mSnoozedNotifications.valueAt(i);
                if (notificationRecord.getSbn().getPackageName().equals(str) && notificationRecord.getUserId() == num.intValue() && Objects.equals(notificationRecord.getSbn().getGroup(), str2)) {
                    arrayList.add(notificationRecord);
                }
            }
        }
    ...
    public boolean cancel(int i, String str) {
        synchronized (this.mLock) {
            ...
            for (int i2 = 0; i2 < size; i2++) {
                ...
                if (notificationRecord.getSbn().getPackageName().equals(str) && notificationRecord.getUserId() == i) {
                    notificationRecord.isCanceled = true;
                }
            }
        }
    ...
    public void repost(String str, boolean z) {
        synchronized (this.mLock) {
            NotificationRecord notificationRecord = (NotificationRecord) this.mSnoozedNotifications.get(str);
    ...
    public void repostGroupSummary(String str, int i, String str2) {
        ...
        synchronized (this.mLock) {
            ...
            while (true) {
                ...
                NotificationRecord notificationRecord = (NotificationRecord) this.mSnoozedNotifications.valueAt(i2);
                if (notificationRecord.getSbn().getPackageName().equals(str) && notificationRecord.getUserId() == i && notificationRecord.getSbn().isGroup() && notificationRecord.getNotification().isGroupSummary() && str2.equals(notificationRecord.getGroupKey())) {
            ...
            if (str3 != null) {
                final NotificationRecord notificationRecord2 = (NotificationRecord) this.mSnoozedNotifications.remove(str3);
                String trimmedString = getTrimmedString(str3);
                this.mPersistedSnoozedNotificationsWithContext.remove(trimmedString);
                this.mPersistedSnoozedNotifications.remove(trimmedString);
                if (notificationRecord2 != null && !notificationRecord2.isCanceled) {
                    new Runnable() { // from class: com.android.server.notification.SnoozeHelper$$ExternalSyntheticLambda5
    ...
    public void clearData(int i, String str) {
        synchronized (this.mLock) {
            for (int size = this.mSnoozedNotifications.size() - 1; size >= 0; size--) {
                final NotificationRecord notificationRecord = (NotificationRecord) this.mSnoozedNotifications.valueAt(size);
                if (notificationRecord.getUserId() == i && notificationRecord.getSbn().getPackageName().equals(str)) {
    ...
    public void writeXml(final TypedXmlSerializer typedXmlSerializer) {
        synchronized (this.mLock) {
            ...
            writeXml(typedXmlSerializer, this.mPersistedSnoozedNotifications, "notification", new Inserter() { // from class: com.android.server.notification.SnoozeHelper$$ExternalSyntheticLambda3
            ...
            writeXml(typedXmlSerializer, this.mPersistedSnoozedNotificationsWithContext, "context", new Inserter() { // from class: com.android.server.notification.SnoozeHelper$$ExternalSyntheticLambda4
                @Override // com.android.server.notification.SnoozeHelper.Inserter
                public final void insert(Object obj) {
    ...
    public static /* synthetic */ void lambda$writeXml$4(long j, TypedXmlSerializer typedXmlSerializer, Long l) {
        if (l.longValue() < j) {
            return;
        }
    ...
    public final void writeXml(TypedXmlSerializer typedXmlSerializer, ArrayMap arrayMap, String str, Inserter inserter) {
