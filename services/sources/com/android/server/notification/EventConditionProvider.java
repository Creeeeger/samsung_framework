...
public class EventConditionProvider extends SystemConditionProviderService {
    ...
    public void dump(PrintWriter printWriter, NotificationManagerService.DumpFilter dumpFilter) {
        ...
        for (int i = 0; i < this.mTrackers.size(); i++) {
            ...
            ((CalendarTracker) this.mTrackers.valueAt(i)).dump("          ", printWriter);
        }
    }
    ...
    public void onScheduleEnabled(boolean z) {
        if (DEBUG) {
            Slog.d("ConditionProviders.ECP", "onScheduleEnabled : " + z);
        }
    }
    ...
    public final void evaluateSubscriptionsW() {
        ...
        synchronized (this.mSubscriptions) {
            ...
            for (int i2 = 0; i2 < this.mTrackers.size(); i2++) {
                ((CalendarTracker) this.mTrackers.valueAt(i2)).setCallback(this.mSubscriptions.isEmpty() ? null : this.mTrackerCallback);
            }
            setRegistered(!this.mSubscriptions.isEmpty());
            Iterator it3 = this.mSubscriptions.iterator();
            long j = 0;
            while (it3.hasNext()) {
                ...
                if (tryParseEventConditionId == null) {
                    arrayList.add(createCondition(uri, i));
                    it = it3;
                } else {
                    if (tryParseEventConditionId.calName == null) {
                        ...
                        while (i3 < this.mTrackers.size()) {
                            CalendarTracker.CheckEventResult checkEvent = ((CalendarTracker) this.mTrackers.valueAt(i3)).checkEvent(tryParseEventConditionId, currentTimeMillis);
                        ...
                        CalendarTracker calendarTracker = (CalendarTracker) this.mTrackers.get(resolveUserId);
                        if (calendarTracker == null) {
                            Slog.w("ConditionProviders.ECP", "No calendar tracker found for user " + resolveUserId);
                            arrayList.add(createCondition(uri, 0));
                        } else {
                    ...
                    if (!checkEventResult.inEvent) {
                        ...
                        i = 0;
                        arrayList.add(createCondition(uri, 1));
                    }
                    it3 = it;
                }
                it3 = it;
    ...
    public final void rescheduleAlarm(long j, long j2) {
        ...
        if (DEBUG) {
            Slog.d("ConditionProviders.ECP", String.format("Scheduling evaluate for %s, in %s, now=%s", SystemConditionProviderService.ts(j2), SystemConditionProviderService.formatDuration(j2 - j), SystemConditionProviderService.ts(j)));
        }
        alarmManager.setExact(0, j2, pendingIntent);
    }

    public PendingIntent getPendingIntent(long j) {
        return PendingIntent.getBroadcast(this.mContext, 1, new Intent(ACTION_EVALUATE).addFlags(268435456).setPackage("android").putExtra("time", j), 201326592);
    }
