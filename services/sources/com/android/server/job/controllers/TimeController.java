...
public final class TimeController extends StateController {
    ...
    public final List mTrackedJobs;
    ...
    public void maybeStartTrackingJobLocked(JobStatus jobStatus, JobStatus jobStatus2) {
        ...
        if (jobStatus.hasTimingDelayConstraint() || jobStatus.hasDeadlineConstraint()) {
            ...
            List list = this.mTrackedJobs;
            ListIterator listIterator = list.listIterator(list.size());
            while (true) {
                if (!listIterator.hasPrevious()) {
                    z = false;
                    break;
                } else if (((JobStatus) listIterator.previous()).getLatestRunTimeElapsed() < jobStatus.getLatestRunTimeElapsed()) {
                    z = true;
                    break;
                }
            }
    ...
    public void checkExpiredDeadlinesAndResetAlarm() {
        ...
        synchronized (this.mLock) {
            long millis = JobSchedulerService.sElapsedRealtimeClock.millis();
            ListIterator listIterator = this.mTrackedJobs.listIterator();
            while (true) {
                ...
                JobStatus jobStatus = (JobStatus) listIterator.next();
                if (jobStatus.hasDeadlineConstraint()) {
                    if (evaluateDeadlineConstraint(jobStatus, millis)) {
                        ...
                        j = jobStatus.getLatestRunTimeElapsed();
                        i = jobStatus.getSourceUid();
                        str = jobStatus.getSourcePackageName();
                        break;
                    }
                }
            }
            ...
            while (it.hasNext()) {
                ...
                if (jobStatus.hasTimingDelayConstraint()) {
                    if (evaluateTimingDelayConstraint(jobStatus, millis)) {
                        ...
                        long earliestRunTime = jobStatus.getEarliestRunTime();
