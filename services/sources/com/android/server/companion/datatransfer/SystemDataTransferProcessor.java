...
public class SystemDataTransferProcessor {
    ...
    public void startSystemDataTransfer(String str, int i, final int i2, final ISystemDataTransferCallback iSystemDataTransferCallback) {
        ...
        Iterator it = this.mSystemDataTransferRequestStore.readRequestsByAssociationId(i, i2).iterator();
        while (true) {
            if (!it.hasNext()) {
                z = false;
                break;
            }
            SystemDataTransferRequest systemDataTransferRequest = (SystemDataTransferRequest) it.next();
            if ((systemDataTransferRequest instanceof PermissionSyncRequest) && systemDataTransferRequest.isUserConsented()) {
                z = true;
                break;
            }
        }
