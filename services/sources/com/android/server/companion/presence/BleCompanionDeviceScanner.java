...
public class BleCompanionDeviceScanner implements AssociationStore.OnChangeListener {
    public static final ScanSettings SCAN_SETTINGS = new ScanSettings.Builder().setCallbackType(6).setScanMode(0).setMatchMode(101).semSetScanFilterRssiThreshold(-75).build();
        ...
        public void onScanResult(int i, ScanResult scanResult) {
            ...
            if (i == 2) {
                if (BleCompanionDeviceScanner.this.mMainThreadHandler.hasNotifyDeviceLostMessages(device)) {
                    ...
                    Log.d("CDM_BleCompanionDeviceScanner", "FIRST_MATCH received during pending MATCH_LOST: return");
                    return;
                } else {
    ...
    public final void notifyDeviceFound(BluetoothDevice bluetoothDevice) {
        ...
        while (it.hasNext()) {
            this.mCallback.onBleCompanionDeviceFound(((AssociationInfo) it.next()).getId());
        }
    }
    ...
    public final void notifyDeviceLost(BluetoothDevice bluetoothDevice) {
        Iterator it = this.mAssociationStore.getAssociationsByAddress(bluetoothDevice.getAddress()).iterator();
        while (it.hasNext()) {
            this.mCallback.onBleCompanionDeviceLost(((AssociationInfo) it.next()).getId());
        }
    }
    ...
    public class MainThreadHandler extends Handler {
        ...
        public void sendNotifyDeviceLostDelayedMessage(BluetoothDevice bluetoothDevice) {
            sendMessageDelayed(obtainMessage(1, bluetoothDevice), 90000L);
        }
