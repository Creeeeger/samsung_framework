...
public class TransportManager {
    ...
    public final void registerTransportsFromPackage(String str, Predicate predicate) {
        try {
            ...
            Slog.e("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "Trying to register transports from package not found " + str));
        }
    }
    ...
    public final boolean isTransportTrusted(ComponentName componentName) {
        if (!this.mTransportWhitelist.contains(componentName)) {
            Slog.w("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "BackupTransport " + componentName.flattenToShortString() + " not whitelisted."));
            return false;
        }
        try {
            ...
            Slog.w("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "Transport package " + componentName.getPackageName() + " not privileged"));
            return false;
    ...
    public final int registerTransport(ComponentName componentName) {
        ...
        try {
            ...
            try {
                ...
                Slog.d("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "Transport " + flattenToShortString + " registered"));
                this.mOnTransportRegisteredListener.onTransportRegistered(name, transportDirName);
                i = 0;
            } catch (RemoteException unused) {
                Slog.e("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "Transport " + flattenToShortString + " died while registering"));
            }
            ...
            Slog.e("BackupTransportManager", addUserIdToLogMessage(this.mUserId, "Couldn't connect to transport " + flattenToShortString + " for registration"));
            this.mTransportConnectionManager.disposeOfTransportClient(transportClient, "TransportManager.registerTransport()");
            return -1;
        }
    }
