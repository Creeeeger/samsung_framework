...
public class WindowMagnificationPromptController {
    ...
    public void showNotificationIfNeeded() {
        if (this.mNeedToShowNotification) {
            ...
            String string = this.mContext.getString(17043390);
            builder.setSmallIcon(R.drawable.ic_check_circle_24px).setContentTitle(this.mContext.getString(17043391)).setContentText(string).setLargeIcon(Icon.createWithResource(this.mContext, R.drawable.ic_clear)).setTicker(this.mContext.getString(17043391)).setOnlyAlertOnce(true).setStyle(new Notification.BigTextStyle().bigText(string)).setDeleteIntent(createPendingIntent(ACTION_DISMISS)).setContentIntent(createPendingIntent(ACTION_TURN_ON_IN_SETTINGS)).setActions(buildTurnOnAction());
    ...
    public final Notification.Action buildTurnOnAction() {
        return new Notification.Action.Builder((Icon) null, this.mContext.getString(17043045), createPendingIntent(ACTION_TURN_ON_IN_SETTINGS)).build();
    }
