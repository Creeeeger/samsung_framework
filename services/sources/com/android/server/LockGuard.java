...
public abstract class LockGuard {
    public static Object[] sKnownFixed = new Object[10];
    public static ArrayMap sKnown = new ArrayMap(0, true);
    ...
    public static void dump(FileDescriptor fileDescriptor, PrintWriter printWriter, String[] strArr) {
        for (int i = 0; i < sKnown.size(); i++) {
            ...
            LockInfo lockInfo = (LockInfo) sKnown.valueAt(i);
            printWriter.println("Lock " + lockToString(keyAt) + XmlUtils.STRING_ARRAY_SEPARATOR);
            for (int i2 = 0; i2 < lockInfo.children.size(); i2++) {
                printWriter.println("  Child " + lockToString(lockInfo.children.valueAt(i2)));
            }
            printWriter.println();
        }
    }
