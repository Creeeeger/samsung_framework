...
public class SliceClientPermissions implements DirtyTracker, DirtyTracker.Persistable {
    ...
    public synchronized void writeTo(XmlSerializer xmlSerializer) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            xmlSerializer.attribute(str2, "authority", ((SliceAuthority) this.mAuths.valueAt(i)).mAuthority);
            xmlSerializer.attribute(str2, "pkg", ((SliceAuthority) this.mAuths.valueAt(i)).mPkg.toString());
    ...
    public class SliceAuthority implements DirtyTracker.Persistable {
        ...
        public boolean hasPermission(List list) {
            ...
            while (it.hasNext()) {
                if (isPathPrefixMatch((String[]) it.next(), (String[]) list.toArray(new String[list.size()]))) {
                    return true;
                }
            }
            return false;
