...
public interface ISurfaceFlingerConfigs extends IBase {
    ...
    OptionalBool hasSyncFramework();

    OptionalBool hasWideColorDisplay();

    OptionalInt64 maxFrameBufferAcquiredBuffers();

    OptionalUInt64 maxVirtualDisplaySize();

    OptionalInt64 presentTimeOffsetFromVSyncNs();

    OptionalBool startGraphicsAllocatorService();

    OptionalBool useContextPriority();

    OptionalBool useHwcForRGBtoYUV();

    OptionalBool useVrFlinger();

    OptionalInt64 vsyncEventPhaseOffsetNs();

    OptionalInt64 vsyncSfEventPhaseOffsetNs();
