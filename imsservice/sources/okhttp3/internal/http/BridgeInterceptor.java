    ...
    public Response intercept(@NotNull Interceptor.Chain chain) throws IOException {
        ...
        if (body2 != null) {
            ...
            if (contentLength != -1) {
                ...
                newBuilder.header(HttpRequest.HEADER_TRANSFER_ENCODING, "chunked");
                newBuilder.removeHeader("Content-Length");
            }
        }
        boolean z = false;
        if (request.header(HttpController.HEADER_HOST) == null) {
            newBuilder.header(HttpController.HEADER_HOST, Util.toHostHeader$default(request.url(), false, 1, null));
        }
        ...
        if (!loadForRequest.isEmpty()) {
            newBuilder.header(HttpController.HEADER_COOKIE, cookieHeader(loadForRequest));
        }
