...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(@NotNull Interceptor.Chain chain) throws IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            Response build = new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(Id.REQUEST_IM_SENDMSG).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
        ...
        private final boolean isEndToEnd(String str) {
            ...
            if (!equals) {
                ...
                if (!equals2) {
                    ...
                    if (!equals3) {
                        equals4 = StringsKt__StringsJVMKt.equals(HttpController.HEADER_PROXY_AUTHORIZATION, str, true);
                        if (!equals4) {
                            equals5 = StringsKt__StringsJVMKt.equals("TE", str, true);
                            if (!equals5) {
                                equals6 = StringsKt__StringsJVMKt.equals("Trailers", str, true);
                                if (!equals6) {
                                    equals7 = StringsKt__StringsJVMKt.equals(HttpRequest.HEADER_TRANSFER_ENCODING, str, true);
                                    if (!equals7) {
