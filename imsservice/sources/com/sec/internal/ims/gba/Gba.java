...
public class Gba {
    ...
    public boolean isKeyExpired(Date date) {
        Date date2 = new Date();
        this.sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
        if (date == null || date.getTime() <= date2.getTime() + (this.threshold * 1000)) {
            return true;
        }
        Log.d(LOG_TAG, "Current Date and time in GMT: " + this.sdf.format(date2) + "  key life time in GMT: " + this.sdf.format(date));
        return false;
    }
    ...
    public int hashCode() {
        GbaStore gbaStore = this.gbaStore;
        return 31 + (gbaStore == null ? 0 : gbaStore.hashCode());
    }
    ...
    public boolean equals(Object obj) {
        ...
        } else if (!gbaStore.equals(gba.gbaStore)) {
            return false;
        }
        return true;
    }
