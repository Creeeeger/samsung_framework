...
public class IARIXmlParser {
    ...
    public boolean parse(InputStream inputStream) {
        ...
        try {
            ...
            return element5.getParentNode() == element || printErrorMessage("Invalid IARI xml: signature node must be a child of iari-authorization");
        } catch (IOException e) {
            return printErrorMessage("Unexpected exception reading IARI xml: " + e.getLocalizedMessage());
        } catch (ParserConfigurationException | SAXException e2) {
            return printErrorMessage("Unexpected exception parsing IARI xml:" + e2.getLocalizedMessage());
        }
    }

    private boolean printErrorMessage(String str) {
        Log.d(LOG_TAG, "iari xml parse error : " + str);
        return false;
    }
