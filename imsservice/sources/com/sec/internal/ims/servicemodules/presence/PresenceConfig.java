...
public class PresenceConfig {
    ...
    PresenceConfig(Context context, int i) {
        this.mPublishTimer = 1200L;
        this.mPublishTimerExtended = 86400L;
        this.mMaxUri = 100;
        this.mDefaultDisc = 0;
        this.mRcsProfile = "";
    ...
    public void load() {
        ...
        long publishTimer = imsProfile.getPublishTimer();
        ...
        if (imsProfile.getBadEventExpiry() == 259200) {
            ...
            this.mBadEventExpiry = imsProfile.getBadEventExpiry();
        }
        this.mPublishErrRetry = imsProfile.getPublishErrRetryTimer();
        Context context = ImsRegistry.getContext();
        this.mTdelayPublish = DmConfigHelper.readInt(context, "t_delay", 5, this.mPhoneId).intValue();
        this.mPublishTimerExtended = imsProfile.getExtendedPublishTimer();
        ...
        if (simMno == Mno.VZW) {
            ...
            IMSLog.s(LOG_TAG, this.mPhoneId, "load: mSourceThrottlePublishFromDM: " + this.mSourceThrottlePublish + ", mSourceThrottleSubscribeFromDM: " + this.mSourceThrottleSubscribe + ", mMaxUriFromDM: " + this.mMaxUri);
            ...
            if (!z) {
                ...
                if (longValue3 > 0) {
                    IMSLog.s(LOG_TAG, this.mPhoneId, "load: change mSourceThrottlePublish to " + longValue3);
                ...
                if (intValue > 0) {
                    IMSLog.s(LOG_TAG, this.mPhoneId, "load: change mMaxUri to " + intValue);
        ...
        if (j == 0) {
            this.mRetryPublishTimer = imsProfile.getPublishExpiry();
        }
        IMSLog.i(LOG_TAG, this.mPhoneId, "load: " + toString());
