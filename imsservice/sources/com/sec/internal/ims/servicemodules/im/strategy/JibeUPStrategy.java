...
public class JibeUPStrategy extends DefaultUPMnoStrategy {
    ...
    public void forceRefreshCapability(Set<ImsUri> set, boolean z, ImError imError) {
        ...
        if (capDiscModule != null) {
            ...
            if (z) {
                capDiscModule.getCapabilities(arrayList, CapabilityRefreshType.ONLY_IF_NOT_FRESH, Capabilities.FEATURE_FT_HTTP | Capabilities.FEATURE_CHAT_SIMPLE_IM, this.mPhoneId);
                return;
            } else {
                if (imError == null || !this.mForceRefreshRemoteCapa.contains(imError)) {
                    return;
                }
                capDiscModule.getCapabilities(arrayList, CapabilityRefreshType.ALWAYS_FORCE_REFRESH, Capabilities.FEATURE_FT_HTTP | Capabilities.FEATURE_CHAT_SIMPLE_IM, this.mPhoneId);
                return;
            }
        }
