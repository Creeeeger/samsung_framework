...
public class FeatureUpdater {
    ...
    public long updateFeatures(int i, ImConfig imConfig) {
        if (!(DmConfigHelper.getImsSwitchValue(this.mContext, DeviceConfigManager.RCS, i) == 1) || imConfig == null) {
        ...
        Log.i(LOG_TAG, "updateFeatures: " + Capabilities.dumpFeature(this.mFeatures));
    ...
    private void updateMnoCustomizedFeatures() {
        ...
        if (iMnoStrategy.isCustomizedFeature(Capabilities.FEATURE_FT_VIA_SMS)) {
            this.mFeatures |= Capabilities.FEATURE_FT_VIA_SMS;
        }
    ...
    private void updateSlmFeatures() {
        ...
        this.mFeatures |= Capabilities.FEATURE_STANDALONE_MSG;
    }
    ...
    private boolean isGlsEnabled(int i) {
        ...
        Log.i(LOG_TAG, "isEnableGls: " + readBoolParam);
        ...
        if (DmConfigHelper.getImsSwitchValue(this.mContext, "im", this.mPhoneId) == 1) {
            if (this.mImConfig.getChatEnabled() || this.mImConfig.getGroupChatEnabled()) {
                if (this.mImConfig.getImMsgTech() == ImConstants.ImMsgTech.SIMPLE_IM) {
                    this.mFeatures |= Capabilities.FEATURE_CHAT_SIMPLE_IM;
                } else {
                    this.mFeatures |= Capabilities.FEATURE_CHAT_CPM;
                }
                if (this.mImConfig.isGroupChatFullStandFwd() || this.mImConfig.isFullSFGroupChat()) {
                    this.mFeatures |= Capabilities.FEATURE_SF_GROUP_CHAT;
                }
                if (this.mImConfig.isJoynIntegratedMessaging() && this.mImModule.isDefaultMessageAppInUse()) {
                    this.mFeatures |= Capabilities.FEATURE_INTEGRATED_MSG;
                }
    ...
    private void updateGlsFeatures() {
        if (DmConfigHelper.getImsSwitchValue(this.mContext, "gls", this.mPhoneId) == 1 && isGlsEnabled(this.mPhoneId)) {
            if (this.mImConfig.getGlsPushEnabled()) {
                this.mFeatures |= Capabilities.FEATURE_GEOLOCATION_PUSH;
                IMnoStrategy iMnoStrategy = this.mMnoStrategy;
                if (iMnoStrategy != null && iMnoStrategy.isCustomizedFeature(Capabilities.FEATURE_GEO_VIA_SMS)) {
                    this.mFeatures |= Capabilities.FEATURE_GEO_VIA_SMS;
                }
            }
            if (this.mImConfig.getGlsPullEnabled()) {
                this.mFeatures |= Capabilities.FEATURE_GEOLOCATION_PULL;
            }
        }
    }
