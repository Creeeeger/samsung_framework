...
public final class SprStrategy extends DefaultRCSMnoStrategy {
    ...
    public void forceRefreshCapability(Set<ImsUri> set, boolean z, ImError imError) {
        ...
        if (z) {
            capDiscModule.getCapabilities(new ArrayList(set), CapabilityRefreshType.ONLY_IF_NOT_FRESH, Capabilities.FEATURE_FT_HTTP | Capabilities.FEATURE_CHAT_SIMPLE_IM, i);
        } else {
            if (imError == null || !this.mForceRefreshRemoteCapa.contains(imError)) {
                return;
            }
            capDiscModule.getCapabilities(new ArrayList(set), CapabilityRefreshType.ALWAYS_FORCE_REFRESH, Capabilities.FEATURE_FT_HTTP | Capabilities.FEATURE_CHAT_SIMPLE_IM, i);
        }
    }
