...
public class CapabilityServiceEventListener {
    ...
    public void registerListener(ICapabilityServiceEventListener iCapabilityServiceEventListener, int i) {
        ...
        RemoteCallbackList<ICapabilityServiceEventListener> remoteCallbackList = this.mListenersList.get(Integer.valueOf(i));
    ...
    public void notifyCapabilitiesChanged(List<ImsUri> list, Capabilities capabilities, ImsUri imsUri, int i) {
        ...
        if (imsUri != null) {
            ...
            while (it.hasNext()) {
                if (TextUtils.equals(it.next().getMsisdn(), imsUri.getMsisdn())) {
                    for (ServiceModuleBase serviceModuleBase : ImsRegistry.getAllServiceModules()) {
                        if (serviceModuleBase instanceof CshModuleBase) {
                            ((CshModuleBase) serviceModuleBase).onRemoteCapabilitiesChanged(capabilities);
                        }
                    }
                }
            }
    ...
    public void notifyEABServiceAdvertiseResult(int i, int i2) {
        RemoteCallbackList<ICapabilityServiceEventListener> remoteCallbackList = this.mListenersList.get(Integer.valueOf(i2));
        try {
            int beginBroadcast = remoteCallbackList.beginBroadcast();
