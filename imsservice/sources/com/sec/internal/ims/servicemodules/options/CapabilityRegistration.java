...
public class CapabilityRegistration {
    ...
    void onRegistered(Context context, ImsRegistration imsRegistration, Map<Integer, ImsRegistration> map, CapabilityConstants.CapExResult capExResult, long j) {
    ...
    void onDeregistering(ImsRegistration imsRegistration, Map<Integer, ImsRegistration> map) {
        int phoneId = imsRegistration.getPhoneId();
    ...
    void onDeregistered(ImsRegistration imsRegistration, Map<Integer, ImsRegistration> map) {
    ...
    void processDeregistered(final ImsRegistration imsRegistration, final Map<Integer, ImsRegistration> map) {
        this.mCapabilityDiscovery.post(new Runnable() { // from class: com.sec.internal.ims.servicemodules.options.CapabilityRegistration$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                CapabilityRegistration.this.lambda$processDeregistered$0(imsRegistration, map);
            }
        });
    }
    ...
    public /* synthetic */ void lambda$processDeregistered$0(ImsRegistration imsRegistration, Map map) {
        ...
        CapabilityDiscoveryModule capabilityDiscoveryModule = this.mCapabilityDiscovery;
        capabilityDiscoveryModule.sendMessageDelayed(capabilityDiscoveryModule.obtainMessage(5, 0, 0, Integer.valueOf(phoneId)), 500L);
    }

    boolean needPublish(Context context, ImsRegistration imsRegistration, int i, Map<Integer, ImsRegistration> map, CapabilityConstants.CapExResult capExResult, long j) {
        ...
        if (!imsRegistration.hasService(SipMsg.EVENT_PRESENCE)) {
            ...
            if (!map.get(Integer.valueOf(i)).getServices().equals(imsRegistration.getServices()) || capExResult == CapabilityConstants.CapExResult.USER_NOT_REGISTERED) {
                return true;
            }
            ...
            boolean isJibeAs = ConfigUtil.isJibeAs(i);
            if (fromName.isKor() && imsRegistration.hasRcsService()) {
                ...
                if (fromName == Mno.ATT && !isJibeAs && !map.get(Integer.valueOf(i)).getEpdgStatus() && imsRegistration.getEpdgStatus() && !this.mCapabilityUtil.isMmtelServiceAvailable(map.get(Integer.valueOf(i)).getRegiRat(), i)) {
                    IMSLog.i(LOG_TAG, i, "needPublish: do publish, epdg handover");
                    return true;
                }
                if (fromName == Mno.TMOUS && !isJibeAs) {
    ...
    void updateOwnCapabilitiesOnRegi(Context context, String str, String str2, int i) {
        ...
        if (ImsRegistry.getBoolean(i, GlobalSettingsConstants.RCS.ENABLE_RCS_EXTENSIONS, false)) {
            Iterator<Map.Entry<String, ?>> it = context.getSharedPreferences("iari_app_association", 0).getAll().entrySet().iterator();
            while (it.hasNext()) {
                String replaceAll = new String(Base64.decode(it.next().getValue().toString(), 0)).replaceAll(":", "%3A");
    ...
    void loadUserLastActiveTimeStamp(Context context, int i) {
        ...
        this.mCapabilityDiscovery.putUserLastActive(i, sharedPreferences.getLong("lastseenactive_" + SimManagerFactory.getImsiFromPhoneId(i), -1L));
    }

    void publish(boolean z, boolean z2, int i, ImsRegistration imsRegistration) {
        if (z) {
        ...
        if (z2 || (imsRegistration.hasService("options") && SimConstants.DSDS_DI.equals(SimUtil.getConfigDualIMS()) && Mno.fromName(imsRegistration.getImsProfile().getMnoName()).isRjil())) {
            IMSLog.i(LOG_TAG, i, "onRegistered : need PUBLISH, expecting EVT_SET_OWN_CAPABILITIES(5) after this");
            if (this.mCapabilityDiscovery.getCapabilityConfig(i) != null && this.mCapabilityDiscovery.getCapabilityConfig(i).usePresence()) {
                if (this.mCapabilityDiscovery.getPresenceModule().getRegiInfoUpdater(i)) {
                    this.mCapabilityDiscovery.removeMessages(5, Integer.valueOf(i));
                    CapabilityDiscoveryModule capabilityDiscoveryModule = this.mCapabilityDiscovery;
                    capabilityDiscoveryModule.sendMessageDelayed(capabilityDiscoveryModule.obtainMessage(5, 0, 0, Integer.valueOf(i)), 500L);
                ...
                capabilityDiscoveryModule2.sendMessageDelayed(capabilityDiscoveryModule2.obtainMessage(53, 0, 0, Integer.valueOf(i)), 500L);
                return;
            }
            this.mCapabilityDiscovery.removeMessages(5, Integer.valueOf(i));
            CapabilityDiscoveryModule capabilityDiscoveryModule3 = this.mCapabilityDiscovery;
            capabilityDiscoveryModule3.sendMessageDelayed(capabilityDiscoveryModule3.obtainMessage(5, 0, 0, Integer.valueOf(i)), 1000L);
        }
    }

    void callContactSync(int i) {
        if (this.mCapabilityDiscovery.getPhonebook().getBlockedInitialContactSyncBeforeRegi()) {
            ...
            if (this.mCapabilityDiscovery.getPhonebook().getLastRefreshTime(i) == 0) {
                if (this.mCapabilityDiscovery.getCapabilityConfig(i).isDisableInitialScan()) {
                    ...
                    this.mCapabilityDiscovery.getPhonebook().setLastRefreshTime(1L, i);
                }
            }
        }
    ...
    void triggerCapexForIncallRegiDeregi(int i, ImsRegistration imsRegistration, ImsRegistration imsRegistration2) {
