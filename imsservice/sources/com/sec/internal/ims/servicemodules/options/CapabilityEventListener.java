...
public class CapabilityEventListener implements ICapabilityEventListener {
    ...
    @Override // com.sec.internal.interfaces.ims.servicemodules.options.ICapabilityEventListener
    ...
    public void onCapabilityUpdate(final List<ImsUri> list, CapabilityConstants.CapExResult capExResult, String str, final OptionsEvent optionsEvent) {
        ...
        if (RcsUtils.isImsSingleRegiRequired(this.mContext, optionsEvent.getPhoneId()) && RcsUtils.isSrRcsOptionsEnabled(this.mContext, optionsEvent.getPhoneId())) {
            ...
            SecImsNotifier.getInstance().onRemoteCapabilityRequest(optionsEvent.getPhoneId(), Uri.parse(list.get(0).uri().toString()), hashSet, new CapabilityExchangeEventListener.OptionsRequestCallback() { // from class: com.sec.internal.ims.servicemodules.options.CapabilityEventListener.1
                ...
                public void onRespondToCapabilityRequestWithError(int i, String str2) {
                    CapabilityEventListener.this.mCapabilityDiscovery.getOptionsModule().sendCapexErrorResponse((ImsUri) list.get(0), optionsEvent.getTxId(), optionsEvent.getPhoneId(), i, str2);
                }
            });
            return;
        }
    ...
    public void onPollingRequested(boolean z, int i) {
        ...
        if (z) {
            if (this.mCapabilityDiscovery.getPollingIntent(i) == null && this.mCapabilityDiscovery.getCapabilityControl(i).isReadyToRequest(i)) {
