...
public class DiagnosisController implements IDiagnosisController {
    ...
    @Override // com.sec.internal.ims.servicemodules.volte2.IDiagnosisController
    public void sendPSDailyInfo() {
        ContentValues contentValues = new ContentValues();
        Log.i(this.LOG_TAG, "CallTypeHistory[" + this.mCsm.mCallTypeHistory + "]");
        ...
        if (this.mCsm.mSession.getCmcType() > 0) {
            ...
            contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_START_TOTAL_COUNT, (Integer) 1);
            if (this.mSession.mEpsFallback) {
                contentValues.put(DiagnosisConstants.DRPT_KEY_EPS_FALLBACK_CALL_COUNT, (Integer) 1);
            }
        }
        ...
        IMSLog.i(this.LOG_TAG, this.mSession.getPhoneId(), "DRPT, storeLogToAgent[" + contentValues.toString() + "]");
    ...
    public void sendPSCallInfo() {
        ...
        if (!this.mCsm.mTelephonyManager.semIsVoNrEnabled(this.mSession.getPhoneId()) && this.mSession.mModule.getNetwork(phoneId) != null && this.mSession.mModule.getNetwork(phoneId).network == 20) {
            contentValues.put(DiagnosisConstants.PSCI_KEY_NETWORK_TYPE, (Integer) 13);
        }
    ...
    protected void dailyInfoCallEnd(ContentValues contentValues) {
        if (needToDailyInfoCallEnd()) {
            if (this.mCsm.mSession.getCmcType() > 0) {
                dailyInfoCallEndForCmc(contentValues);
                return;
            }
            if (this.mCsm.mTelephonyManager.semIsVoNrEnabled(this.mSession.getPhoneId()) && isNrWhenCallEnd(this.mSession.mPhoneId)) {
    ...
    private boolean needToDailyInfoCallEnd() {
        return (this.mSession.getEndReason() == 8 || this.mSession.getCallProfile().hasCSFBError()) ? false : true;
    }
    ...
    private void dailyInfoCallEndForNR(ContentValues contentValues) {
        if (ImsCallUtil.isVideoCall(this.mSession.getCallProfile().getCallType())) {
            contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_END_VIDEO_COUNT, (Integer) 1);
        } else if (ImsCallUtil.isE911Call(this.mSession.getCallProfile().getCallType())) {
            contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_END_EMERGENCY_COUNT, (Integer) 1);
        } else {
            contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_END_VOICE_COUNT, (Integer) 1);
        }
        if (isCallDrop(this.mCsm.errorCode)) {
            contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_END_FAIL_COUNT, (Integer) 1);
        }
        contentValues.put(DiagnosisConstants.DRPT_KEY_VONR_END_TOTAL_COUNT, (Integer) 1);
    }
    ...
    private boolean isOutgoingFail() {
        ...
        if (previousState != callStateMachine.mOutgoingCall) {
            ...
            CallStateMachine callStateMachine2 = this.mCsm;
            if (previousState2 != callStateMachine2.mAlertingCall && (callStateMachine2.getPreviousState() != this.mCsm.mReadyToCall || !this.mSession.getCallProfile().isMOCall())) {
                return false;
            }
        }
        return true;
