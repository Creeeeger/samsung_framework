...
public class EucTestEventsFactory implements IEucTestEventsFactory {
    ...
    @Override // com.sec.internal.ims.servicemodules.euc.test.IEucTestEventsFactory
    ...
    public IEucSystemRequest createSystemRequest(Intent intent) {
        ...
        FlatBufferBuilder flatBufferBuilder = new FlatBufferBuilder(0);
        flatBufferBuilder.finish(SystemMessage.createSystemMessage(flatBufferBuilder, buildBaseMessage(flatBufferBuilder, extras), flatBufferBuilder.createString(makeStrNotNull(string)), flatBufferBuilder.createString(makeStrNotNull(string2))));
    ...
    private int buildBaseMessage(FlatBufferBuilder flatBufferBuilder, Bundle bundle) {
        return BaseMessage.createBaseMessage(flatBufferBuilder, bundle.getInt(EucTestIntent.Extras.HANDLE), flatBufferBuilder.createString(bundle.getString("id")), flatBufferBuilder.createString(bundle.getString("remote_uri")), getTimestamp(bundle));
    }
    ...
    private int[] buildTextLangPairList(FlatBufferBuilder flatBufferBuilder, List<String> list, List<String> list2) {
        if (list2.isEmpty()) {
            Preconditions.checkArgument(list.size() == 1, "If more than one element is presented a language (lang) attribute must be present with the two letter language codes according to the ISO 639-1");
            int createString = flatBufferBuilder.createString(makeStrNotNull(list.get(0)));
        ...
        Preconditions.checkArgument(list.size() == list2.size(), "Text and language size does not match");
        int[] iArr = new int[list.size()];
        for (int i = 0; i < list.size(); i++) {
            iArr[i] = TextLangPair.createTextLangPair(flatBufferBuilder, flatBufferBuilder.createString(makeStrNotNull(list.get(i))), flatBufferBuilder.createString(makeStrNotNull(list2.get(i))));
        }
        return iArr;
    }
    ...
    private <T, S> S translateMessageToRequest(T t, TypeTranslator<T, S> typeTranslator) {
        try {
            ...
            this.logInvalidIntent(e);
            return null;
        }
    }

    private void logInvalidIntent(TranslationException translationException) {
        Log.e(LOG_TAG, LOG_MSG_INVALID_INTENT + translationException.getMessage());
    }
    ...
    private boolean checkNoBaseExtras(Bundle bundle) {
        return (bundle.containsKey(EucTestIntent.Extras.HANDLE) && bundle.containsKey("id") && bundle.containsKey("remote_uri")) ? false : true;
    }

    private boolean checkNoSystemRequestExtras(Bundle bundle) {
        return (bundle.containsKey(EucTestIntent.Extras.SYSTEM_TYPE) && bundle.containsKey(EucTestIntent.Extras.SYSTEM_DATA)) ? false : true;
    }

    private boolean checkNoUserConsentExtras(Bundle bundle) {
        return (bundle.containsKey(EucTestIntent.Extras.TITLE) && bundle.containsKey("message") && bundle.containsKey(EucTestIntent.Extras.SUBSCRIBER_IDENTITY)) ? false : true;
    }
