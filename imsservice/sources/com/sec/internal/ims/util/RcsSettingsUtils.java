...
public class RcsSettingsUtils {
    ...
    private static RcsSettingsUtils sInstance;
    private String LOG_TAG = RcsSettingsUtils.class.getSimpleName();
    ...
    private static final Map<String, String[]> sCountryCodes = new HashMap<String, String[]>() { // from class: com.sec.internal.ims.util.RcsSettingsUtils.1
...

        {
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_ANTARCTICA, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_BOUVET_ISLAND, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_COCOS_ISLANDS, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_ECUADOR, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_SOUTH_GEORGIA, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_HEARD_MCDONALD_ISLANDS, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_PALESTINE, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_RUSSIA, new String[]{"7", "8"});
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_SVALBARD_JAN_MAYEN, null);
            ...
            put(RcsSettingsUtils.ISO_ALPHA2_FRENCH_SOUTHERN_TERRITORIES, null);
    ...
    private RcsSettingsUtils(Context context) {
        this.mContext = context;
    }

    public void writeBoolean(String str, boolean z) {
        writeParameter(str, Boolean.toString(z));
    }
    ...
    public String readParameter(String str) {
        ...
        Cursor query = this.mContext.getContentResolver().query(sDatabaseUri, null, WHERE_CLAUSE, new String[]{str}, null);
        if (query != null) {
            try {
                if (query.moveToFirst()) {
                    String string = query.getString(query.getColumnIndexOrThrow("value"));
                    Log.d(this.LOG_TAG, String.format("readParameter: %s = %s", str, string));
                    query.close();
                    return string;
                }
            } catch (Throwable th) {
    ...
    public int writeParameter(String str, String str2) {
        if (sInstance == null || str2 == null) {
            return 0;
        }
        IMSLog.s(this.LOG_TAG, "writeParameter: " + str + AuthenticationHeaders.HEADER_PRARAM_SPERATOR + str2);
        ContentValues contentValues = new ContentValues();
        contentValues.put("value", str2);
        try {
            return this.mContext.getContentResolver().update(sDatabaseUri, contentValues, WHERE_CLAUSE, new String[]{str});
        } catch (SQLException e) {
            Log.e(this.LOG_TAG, "SQL exception while insert Settings Parameter. " + e);
            return 0;
    ...
    public void loadCCAndAC() {
        String simCountryIso = ((TelephonyManager) this.mContext.getSystemService(PhoneConstants.PHONE_KEY)).getSimCountryIso();
        if (TextUtils.isEmpty(simCountryIso)) {
            Log.d(this.LOG_TAG, "setCCAndTc Can't read country code from SIM");
            return;
        }
        String[] strArr = sCountryCodes.get(simCountryIso);
        if (strArr == null) {
            Log.d(this.LOG_TAG, "there is no country code info about this country: " + simCountryIso);
            return;
        }
        String str = strArr[0];
        Log.d(this.LOG_TAG, "country code " + str);
        if (!TextUtils.isEmpty(str)) {
            setCountryCode(COUNTRY_CODE_PREFIX.concat(str));
        }
        if (strArr.length > 1) {
            String str2 = strArr[1];
            Log.d(this.LOG_TAG, "country area code " + str2);
            if (TextUtils.isEmpty(str2)) {
                return;
            }
            setCountryAreaCode(str2);
        }
    }
    ...
    public void updateSettings() {
        ...
        for (ImsRegistration imsRegistration : registrationInfo) {
            ...
            while (it.hasNext()) {
                ImsUri uri = ((NameAddr) it.next()).getUri();
                String extractNumberFromUri = PhoneUtils.extractNumberFromUri(uri.toString());
                if (extractNumberFromUri != null) {
                    setMyContactId(extractNumberFromUri);
                    IMSLog.s(this.LOG_TAG, String.format("Load Number: %s from uri: %s", extractNumberFromUri, uri.toString()));
                    return;
                }
            }
        }
    ...
    public void updateTapiSettings() {
        ...
        boolean z = DmConfigHelper.getImsSwitchValue(this.mContext, DeviceConfigManager.RCS, SimUtil.getActiveDataPhoneId()) == 1;
        IMSLog.s(this.LOG_TAG, "check isRcsEnabled:" + z);
        setDefaultMessagingMethod(z ? CommonServiceConfiguration.MessagingMethod.AUTOMATIC : CommonServiceConfiguration.MessagingMethod.NON_RCS);
        setServiceActivated(z);
        writeBoolean("ServiceAvailable", z);
    }
    ...
    public static synchronized RcsSettingsUtils getInstance(Context context) {
        RcsSettingsUtils rcsSettingsUtils;
        synchronized (RcsSettingsUtils.class) {
            if (sInstance == null) {
                RcsSettingsUtils rcsSettingsUtils2 = new RcsSettingsUtils(context);
    ...
    public void setServiceActivated(boolean z) {
        writeBoolean("ServiceActivated", z);
    }

    public void setConfigValid(boolean z) {
        writeBoolean("ConfigurationValidity", z);
    }
    ...
    public void setCountryCode(String str) {
        writeParameter("MyCountryCode", str);
    }

    public void setCountryAreaCode(String str) {
        writeParameter("CountryAreaCode", str);
    }

    public void setMyContactId(String str) {
        writeParameter("MyContactId", str);
    }

    public void setDefaultMessagingMethod(CommonServiceConfiguration.MessagingMethod messagingMethod) {
        writeParameter("DefaultMessagingMethod", messagingMethod.toString());
    }
    ...
    private class SimEventListener implements ISimEventListener {
        ...
        public void onReady(int i, boolean z) {
            Log.d(RcsSettingsUtils.this.LOG_TAG, "onReady: subId=" + i + " absent=" + z);
            if (z) {
                return;
            }
