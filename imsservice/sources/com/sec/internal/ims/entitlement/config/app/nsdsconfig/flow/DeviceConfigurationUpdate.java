...
public class DeviceConfigurationUpdate extends NSDSAppFlowBase {
    ...
    public void performDeviceConfigRetrieval(int i, int i2) {
        ...
        if (nextOperation == -1) {
            ...
            queueOperation(nextOperation, null);
        }
    }
    ...
    private int getActionTrigger() {
        ...
        if (checkGroupName(this.mDeviceGroup)) {
            ...
            intInDe = NSDSSharedPrefHelper.getIntInDe(this.mContext, NSDSNamespaces.NSDSSharedPref.NAME_SHARED_PREF_CONFIG, deviceId, NSDSNamespaces.NSDSSharedPref.PREF_ACTION_TRIGGER);
            IMSLog.i(LOG_TAG, "getActionTrigger (saved) :  " + intInDe);
            if (intInDe == -1) {
                intInDe = 0;
            } else if (intInDe == 0) {
                intInDe = 1;
        ...
        NSDSSharedPrefHelper.saveActionTrigger(this.mContext, deviceId, intInDe);
        IMSLog.i(LOG_TAG, "getActionTrigger (updated) :  " + intInDe);
        return intInDe;
    }
    ...
    private void retrieveDeviceConfiguration() {
        ...
        configurationRetrievalWithSIM.retriveDeviceConfiguration(mnoNsdsConfigStrategy != null ? mnoNsdsConfigStrategy.getEntitlementServerUrl(deviceId) : null, this.mDeviceGroup, vIMSIforSIMDevice, NSDSHelper.getMSISDNFromSIM(this.mContext, this.mBaseFlowImpl.getSimManager().getSubscriptionId()), "1.0", NSDSDatabaseHelper.getConfigVersion(this.mContext, imsi), NSDSHelper.getTACfromCellInfo(this.mContext), getActionTrigger());
    }
    ...
    private void handleResponseGetDeviceConfig(Bundle bundle, boolean z) {
        ...
        if (bundle != null && httpErrRespCode <= 0 && httpErrRespReason == null) {
            ...
            if (responseManageConnectivity != null) {
                int i = responseManageConnectivity.responseCode;
                if (i != 1000 || (str = responseManageConnectivity.deviceConfig) == null) {
                    httpErrRespCode = i;
                } else {
        ...
        notifyNSDSFlowResponse(z2, NSDSNamespaces.NSDSMethodNamespace.MANAGE_CONNECTIVITY, z ? 1 : 3, httpErrRespCode);
    }
    ...
    private void persistDeviceConfig(ResponseManageConnectivity responseManageConnectivity, DeviceConfiguration deviceConfiguration, String str, boolean z) {
        if (responseManageConnectivity != null) {
            IMSLog.i(LOG_TAG, "ResponseManageConnectivity : messageId:" + responseManageConnectivity.messageId + " responseCode:" + responseManageConnectivity.responseCode + " serviceNames:" + responseManageConnectivity.serviceNames + " deviceConfig:" + hidePrivateInfoFromMsg(responseManageConnectivity.deviceConfig));
    ...
    protected IMnoNsdsConfigStrategy getMnoNsdsConfigStrategy() {
    ...
    private String hidePrivateInfoFromMsg(String str) {
        return TextUtils.isEmpty(str) ? "NO CONFIG" : !Debug.isProductShip() ? str : str.replaceAll("sip:+[0-9+-]+", "sip:xxxxxxxxxxx").replaceAll("tel:+[0-9+-]+", "tel:xxxxxxxxxxx");
    }
}
