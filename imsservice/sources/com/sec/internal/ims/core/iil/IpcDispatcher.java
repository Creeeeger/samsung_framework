...
public abstract class IpcDispatcher {
    ...
    abstract void handleChannelProxyDead(long j);

    abstract void handleSendIpc(byte[] bArr);

    abstract void initChannel();

    abstract void resetProxy();
    ...
    void processResponse(byte[] bArr, int i) {
    ...
    protected void notifyChannelProxyDied(long j) {
        Iterator<Registrant> it = this.mRegistrants.iterator();
        while (it.hasNext()) {
    ...
    protected void handleChannelProxyExceptionForRR(String str, Exception exc) {
        IMSLog.e(LOG_TAG, this.mSlotId, str + ": " + exc);
        resetProxy();
        publishChannelProxyDeadEvent(this.mChannelProxyCookie.incrementAndGet());
    }

    protected void publishChannelProxyDeadEvent(long j) {
        SecChannelHandler secChannelHandler = this.mSecChannelHandler;
        secChannelHandler.sendMessageDelayed(secChannelHandler.obtainMessage(1, Long.valueOf(j)), 4000L);
    }
}
