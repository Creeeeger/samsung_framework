...
class StackRequestBuilderUtil {
    ...
    private static Map<Integer, Mno> translateMnoInverseMap;
    ...
    static {
        ...
        translateMnoMap.put(Mno.EPIC_CYPRUS, 308);
        translateMnoMap.put(Mno.MTEL_BOSNIA, Integer.valueOf(MNO.MTEL_BA));
        ...
        translateMnoMap.put(Mno.UTS_CURACAO, 253);
        ...
        translateMnoMap.put(Mno.ARQIA_BRAZIL, 309);
        translateMnoMap.put(Mno.GENERIC_IR92, 222);
        translateMnoMap.put(Mno.TANGO_US, Integer.valueOf(MNO.TANGO_US));
        translateMnoInverseMap = (Map) translateMnoMap.entrySet().stream().collect(Collectors.toMap(new DiagnosisConstants$$ExternalSyntheticLambda1(), new Function() { // from class: com.sec.internal.ims.core.handler.secims.StackRequestBuilderUtil$$ExternalSyntheticLambda0
            @Override // java.util.function.Function
            public final Object apply(Object obj) {
                return (Mno) ((Map.Entry) obj).getKey();
            }
        }));
    }
    ...
    static List<Integer> translateFeatureTag(long j) {
        ...
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_MMTEL)) {
            Log.i(str, "feature Tag MMTEL");
            arrayList.add(9);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_MMTEL_VIDEO)) {
        ...
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_IPCALL)) {
            Log.i(str, "feature Tag IPCALL");
            arrayList.add(0);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_IPCALL_VIDEO)) {
            Log.i(str, "feature Tag IPCALL VIDEO");
            arrayList.add(1);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_IPCALL_VIDEO_ONLY)) {
            Log.i(str, "feature Tag IPCALL Video Only");
            arrayList.add(2);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_PRESENCE_DISCOVERY)) {
            Log.i(str, "feature Tag PRESENCE_DISCOVERY");
            arrayList.add(19);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_CHAT_CPM)) {
            Log.i(str, "feature Tag CHAT");
            arrayList.add(20);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_CHAT_SIMPLE_IM)) {
            Log.i(str, "feature Tag SESSION_MODE_MSG");
            arrayList.add(20);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_SF_GROUP_CHAT)) {
            Log.i(str, "feature Tag SF_GROUP_CHAT");
            arrayList.add(21);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT)) {
            Log.i(str, "feature Tag FT");
            arrayList.add(22);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT_HTTP)) {
            Log.i(str, "feature Tag FT_HTTP");
            arrayList.add(25);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT_HTTP_EXTRA)) {
            Log.i(str, "feature Tag FT_HTTP_EXTRA");
            arrayList.add(33);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT_STORE)) {
            Log.i(str, "feature Tag FT_STORE");
            arrayList.add(24);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT_THUMBNAIL)) {
            Log.i(str, "feature Tag FT_THUMBNAIL");
            arrayList.add(23);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_ISH)) {
            Log.i(str, "feature Tag ISH");
            arrayList.add(18);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_VSH)) {
            Log.i(str, "feature Tag VSH");
            arrayList.add(3);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_VSH_OUTSIDE_CALL)) {
            Log.i(str, "feature Tag VSH_OUTSIDE_CALL");
            arrayList.add(26);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_INTEGRATED_MSG)) {
            Log.i(str, "feature Tag INTEGRATED_MSG");
            arrayList.add(27);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_STANDALONE_MSG)) {
            Log.i(str, "feature Tag STANDALONE_MSG");
            arrayList.add(10);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_SOCIAL_PRESENCE)) {
            Log.i(str, "feature Tag SOCIAL_PRESENCE");
            arrayList.add(28);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_GEOLOCATION_PUSH)) {
            Log.i(str, "feature Tag GEOLOCATION_PUSH");
            arrayList.add(31);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_GEOLOCATION_PULL)) {
            Log.i(str, "feature Tag GEOLOCATION_PULL");
            arrayList.add(29);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_GEOLOCATION_PULL_FT)) {
        ...
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_STICKER)) {
            Log.i(str, "feature Tag STICKER");
            arrayList.add(37);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_FT_VIA_SMS)) {
            Log.i(str, "feature Tag FT_VIA_SMS");
            arrayList.add(38);
        }
        if (CapabilityUtil.hasFeature(j, Capabilities.FEATURE_GEO_VIA_SMS)) {
    ...
    static List<Integer> translateExtraHeader(FlatBufferBuilder flatBufferBuilder, HashMap<?, ?> hashMap) {
        ...
        while (it.hasNext()) {
            ...
            arrayList.add(Integer.valueOf(Pair.createPair(flatBufferBuilder, flatBufferBuilder.createString(obj), flatBufferBuilder.createString(hashMap.get(obj).toString()))));
        }
        return arrayList;
    }
}
