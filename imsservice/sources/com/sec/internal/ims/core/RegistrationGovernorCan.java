...
public class RegistrationGovernorCan extends RegistrationGovernorBase {
    ...
    public boolean isReadyToRegister(int i) {
        ...
        if (this.mVsm != null && (((mno = this.mMno) == Mno.TELUS || mno == Mno.KOODO) && this.mTask.getProfile().hasEmergencySupport() && this.mTask.getState() == RegistrationConstants.RegisterTaskState.REGISTERED && this.mVsm.hasEmergencyCall(this.mPhoneId))) {
            return false;
        }
    ...
    public Set<String> filterService(Set<String> set, int i) {
        ...
        if (this.mMno == Mno.BELL) {
            if (i != 18 && (!NetworkUtil.isMobileDataOn(this.mContext) || SlotBasedConfig.getInstance(this.mPhoneId).isDataUsageExceeded())) {
                ...
                boolean z2 = ImsConstants.SystemSettings.DATA_ROAMING.get(this.mContext, ImsConstants.SystemSettings.DATA_ROAMING_UNKNOWN) == ImsConstants.SystemSettings.ROAMING_DATA_ENABLED;
                Log.i(LOG_TAG, "isNetworkInRoaming " + isNetworkRoaming + " isDataRoamingOn " + z2);
                if (isNetworkRoaming && !z2) {
        ...
        if (this.mConfigModule.isValidAcsVersion(this.mPhoneId)) {
            ...
            if (!z) {
                for (String str2 : ImsProfile.getChatServiceList()) {
                    removeService(filterService, str2, "chatservice off");
                }
            }
        }
