...
public class DmProfileLoader {
    ...
    private static synchronized ImsProfile getFromDmStorage(Context context, ImsProfile imsProfile, int i) {
        ImsProfile imsProfile2;
        int indexOf;
        synchronized (DmProfileLoader.class) {
            mValueList.clear();
            mLboPcscfAddrList.clear();
            mLboPcscfPort = -1;
            mIsKorOp = Mno.fromName(imsProfile.getMnoName()).isKor();
            Log.e(LOG_TAG, "mIsKorOp: " + mIsKorOp);
            for (Map.Entry<String, String> entry : DmConfigHelper.read(context, "omadm/*", i).entrySet()) {
                String key = entry.getKey();
                String value = entry.getValue();
                String replaceFirst = key.replaceFirst(DmConfigModule.DM_PATH, "");
                setValueWithUri(replaceFirst, value);
                if (replaceFirst != null && !TextUtils.isEmpty(value) && replaceFirst.contains("./3GPP_IMS/LBO_P-CSCF_Address") && replaceFirst.endsWith("Address")) {
                    if (value.indexOf(91) >= 0 && value.indexOf(93) >= 0) {
                        if (value.indexOf("]:") > 0 && (indexOf = value.indexOf("]:")) >= 0) {
                            mLboPcscfPort = Integer.parseInt(value.substring(indexOf + 2));
                        }
                        mLboPcscfAddrList.add(value.substring(value.indexOf(91) + 1, value.indexOf(93)));
                    } else {
                        int indexOf2 = value.indexOf(58);
                        if (indexOf2 >= 0) {
                            mLboPcscfPort = Integer.parseInt(value.substring(indexOf2 + 1));
                            value = value.substring(0, indexOf2);
                        }
                        mLboPcscfAddrList.add(value);
                    }
                    if (mLboPcscfPort == -1) {
            ...
            imsProfile2 = new ImsProfile(imsProfile);
    ...
    private static void updateDbInfoToProfile(ImsProfile imsProfile) {
        ...
        if (getIntValue("72") >= 0) {
            imsProfile.put("volte_service_status", Boolean.valueOf(getIntValue("72") == 1));
        }
