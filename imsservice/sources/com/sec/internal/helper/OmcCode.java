...
public class OmcCode {
    ...
    private static String getTSS2_0_NWCode(Mno mno) {
        ...
        return (!TextUtils.equals(countryIso, representativeCountryISO.toUpperCase()) || TextUtils.isEmpty(str) || "EUX".equalsIgnoreCase(str) || "EUY".equalsIgnoreCase(str)) ? getOpenBuyerByCountryIso(representativeCountryISO) : str;
    }

    private static String getOmcCodeByNwPath(int i, boolean z) {
        String nWPath = getNWPath(SimUtil.isMultiSimSupported() ? i : 0);
        if (nWPath.contains("/")) {
            for (String str : nWPath.split("/")) {
    ...
    public static String getLastOmcNwCode(int i) {
        String str = SemSystemProperties.get(i == 0 ? PERSIST_LAST_OMCNW_CODE : PERSIST_LAST_OMCNW_CODE2);
        if (!SimUtil.isSoftphoneEnabled()) {
            return str;
        }
        String str2 = get();
        IMSLog.i(LOG_TAG, i, "Force Fix Softphone NwCode Using : " + str2);
        return str2;
    }
    ...
    public static boolean isJPNOmcCode() {
        return "DCM".equals(get()) || "KDI".equals(get()) || "UQM".equals(get()) || "JCO".equals(get()) || "SJP".equals(get()) || "XJP".equals(get()) || "RKT".equals(get()) || "SBM".equals(get());
    }

    public static boolean isKDIMhs() {
        return Boolean.parseBoolean(SemFloatingFeature.getInstance().getString(SecFeature.FLOATING.MHS_DONGLE)) && ("KDI".equals(get()) || "UQM".equals(get()));
    }
    ...
    public enum EUR_TSS2_0_COUNTRYCODE {
        ...
        EUR_TSS2_0_COUNTRYCODE(String str) {
            this.openBuyer = str;
        }
