...
public class RegiConfig {
    ...
    private void setPrivateUserIdentity() {
        this.mPrivateUserIdentity = RcsConfigurationHelper.readStringParam(this.mContext, ImsUtil.getPathWithPhoneId(ConfigConstants.ConfigTable.PRIVATE_USER_IDENTITY, this.mPhoneId), null);
    }
    ...
    private void setHomeNetworkDomain() {
        this.mHomeNetworkDomain = RcsConfigurationHelper.readStringParam(this.mContext, ImsUtil.getPathWithPhoneId(ConfigConstants.ConfigTable.HOME_NETWORK_DOMAIN_NAME, this.mPhoneId), null);
    }
    ...
    private void setAppRealm() {
        this.mAppRealm = RcsConfigurationHelper.readStringParam(this.mContext, ImsUtil.getPathWithPhoneId("realm", this.mPhoneId), null);
    }
    ...
    public Bundle getLboPcscfAddressAndIpType() {
        String str;
        String str2;
        if (CollectionUtils.isNullOrEmpty(this.mLboPcscfAddress)) {
            str = null;
            str2 = null;
            ...
            str = this.mLboPcscfAddress.get(this.mPcscfIdx);
            str2 = this.mIpType.get(this.mPcscfIdx);
            IMSLog.d(LOG_TAG, this.mPhoneId, "getRcsPcscfAddress mPcscfIdx:" + this.mPcscfIdx + " lboPcscfAddress:" + str + " ipType:" + str2);
            this.mPcscfIdx = (this.mPcscfIdx + 1) % this.mLboPcscfAddress.size();
        } else {
            str = this.mLboPcscfAddress.get(0);
            str2 = this.mIpType.get(0);
        }
        Bundle bundle = new Bundle();
        bundle.putString("address", str);
        bundle.putString(ConfigConstants.ConfigTable.LBO_PCSCF_ADDRESS_TYPE, str2);
        return bundle;
    }
}
