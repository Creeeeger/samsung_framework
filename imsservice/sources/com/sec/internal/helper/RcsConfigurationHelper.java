...
public final class RcsConfigurationHelper {
    ...
    private static void updateImEnabledServices(int i, ConfigData configData, List<String> list, String str) {
        ...
        if (ImsProfile.isRcsUp2Profile(str)) {
            if (booleanValue && !TextUtils.isEmpty(readString)) {
                list.add("ft_http");
            }
        } else {
            if (booleanValue) {
    ...
    protected static Uri getUriParamWithPhoneId(Uri uri, String str) {
        ...
        if (str.contains("#simslot0")) {
            ...
            simSlotPriority = str.contains("#simslot1") ? 1 : SimUtil.getSimSlotPriority();
        }
        return Uri.parse(uri + str.replaceAll("#simslot\\d", "")).buildUpon().fragment("simslot" + simSlotPriority).build();
    }
    ...
    public static boolean isUp2NonTransitional(String str, int i) {
        ...
        if ("NAGuidelines".equals(str)) {
            ...
            if (isImsSingleRegiRequired(context, i) && isGoogDmaPackageInuse(context, i) && isSrUseAcsForRcs(context, i)) {
                ...
                IMSLog.i(LOG_TAG, i, String.format(Locale.US, "isUp2NonTransitional: rcc [%s] => [%s]", rcsClientConfiguration, readStringParamWithPath));
                str = readStringParamWithPath;
            } else if (!string.isEmpty()) {
                str = string;
            }
        }
        return ImsProfile.isRcsUp2Profile(str) && !ImsProfile.isRcsUpTransitionProfile(string);
    }
    ...
    public static ImConstants.FtMech getFtDefaultTech(ConfigData configData, String str, int i) {
        ...
        return DiagnosisConstants.RCSM_KEY_MSRP.equals(configData.readString(ConfigConstants.ConfigTable.IM_FT_DEFAULT_MECH, DiagnosisConstants.RCSM_KEY_MSRP)) ? ImConstants.FtMech.MSRP : ImConstants.FtMech.HTTP;
    }
}
