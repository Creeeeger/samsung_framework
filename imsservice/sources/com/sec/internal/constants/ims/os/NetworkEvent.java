...
public class NetworkEvent {
    ...
    private boolean equalsInternal(Object obj, boolean z) {
        ...
        if ((z || (this.isEpdgConnected == networkEvent.isEpdgConnected && this.isEpdgAvailable == networkEvent.isEpdgAvailable)) && this.isPsOnlyReg == networkEvent.isPsOnlyReg) {
            return (TextUtils.isEmpty(this.operatorNumeric) || TextUtils.equals(this.operatorNumeric, networkEvent.operatorNumeric)) && this.isDataStateConnected == networkEvent.isDataStateConnected;
        }
        return false;
    }
    ...
    public String changedEvent(NetworkEvent networkEvent) {
        ...
        if (this.isDataRoaming != networkEvent.isDataRoaming) {
            str = "Changed Event: DataRoaming(" + this.isDataRoaming + "=>" + networkEvent.isDataRoaming + "), ";
        }
    ...
    public boolean isEpdgHOEvent(NetworkEvent networkEvent) {
        ...
        if (i2 == 18 && ((i = networkEvent.network) == 13 || i == 14)) {
            Log.d(LOG_TAG, "isEpdgHOEvent: From LTE to IWLAN.");
            return true;
        }
        if (i2 != networkEvent.network || this.isWifiConnected == networkEvent.isWifiConnected) {
            return false;
        }
    ...
    public static NetworkEvent buildNetworkEvent(int i, int i2, int i3, int i4, boolean z, boolean z2, boolean z3, NetworkEvent networkEvent, NetworkState networkState) {
        boolean z4;
        int i5;
        boolean z5 = i2 == 0 || networkState.getDataRegState() != 0;
        boolean z6 = networkState.getVoiceRegState() != 0;
        int voiceNetworkType = networkState.getVoiceNetworkType();
        boolean isDataRoaming = networkState.isDataRoaming();
        boolean isDataConnectedState = networkState.isDataConnectedState();
        boolean isVoiceRoaming = networkState.isVoiceRoaming();
        String operatorNumeric = networkState.getOperatorNumeric();
        VoPsIndication vopsIndication = networkState.getVopsIndication();
        boolean isPsOnlyReg = networkState.isPsOnlyReg();
        int blurNetworkType = blurNetworkType(i2);
        if (SimUtil.getSimMno(i) == Mno.TMOUS && is2GNetworkInCall(blurNetworkType, z5, z6, i3, i4)) {
            z4 = false;
        ...
        NetworkEvent networkEvent2 = new NetworkEvent(i5, voiceNetworkType, z4, z6, isDataRoaming, isVoiceRoaming, vopsIndication, z, z2, z3, operatorNumeric, isPsOnlyReg, isDataConnectedState);
        networkEvent2.isVopsUpdated = networkEvent2.isVopsUpdated(networkEvent) != VopsState.KEEP;
