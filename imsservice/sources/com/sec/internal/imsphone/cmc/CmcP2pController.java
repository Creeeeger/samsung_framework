...
public class CmcP2pController extends Handler {
    ...
    private final boolean mIsEnableP2pFeature;
    ...
    public CmcP2pController(Looper looper, ICmcConnectivityController iCmcConnectivityController, IRegistrationManager iRegistrationManager, IpServiceManager ipServiceManager) {
        super(looper);
        this.mIsEnableP2pFeature = false;
    ...
    public void stopP2p() {
        Log.i(LOG_TAG, "stopP2p: curDeviceType: " + this.mDeviceType + " -> None");
    ...
    public void setCmcActivation(boolean z) {
        if (z) {
            ...
            IMSLog.i(LOG_TAG, "connected, mPrevAct: " + this.mPrevAct);
            Settings.Global.putInt(this.mContext.getContentResolver(), WD_CMC_ACTIVATION, 1);
            return;
        }
        IMSLog.i(LOG_TAG, "disconnected, mPrevAct: " + this.mPrevAct);
        if (this.mPrevAct != -1) {
    ...
    public boolean isExistP2pConnection() {
        Log.i(LOG_TAG, "isExistP2pConnection, mWifiDirectIsConnect: " + this.mWifiDirectIsConnect);
        return this.mWifiDirectIsConnect;
    }

    public ICmcConnectivityController.DeviceType getDeviceType() {
        Log.i(LOG_TAG, "getDeviceType, mDeviceType: " + this.mDeviceType);
    ...
    public void onRegistered(int i) {
        Log.i(LOG_TAG, "onRegistered, cmcType: " + i);
        if (i == 7 || i == 8) {
    ...
    private void imsRegister(String str, String str2) {
        IMSLog.i(LOG_TAG, "imsRegister(), mDeviceType: " + this.mDeviceType);
        for (ImsProfile imsProfile : ImsProfileLoader.getProfileListWithMnoName(this.mContext, "MDMN", 0)) {
            if ("SamsungCMC_P2P".equals(imsProfile.getName())) {
                ...
                String str3 = LOG_TAG;
                Log.i(str3, "mPrimaryDuid: " + this.mPrimaryDuid);
                Log.i(str3, "mOwnDeviceId: " + this.mOwnDeviceId);
            ...
            for (NetworkInterface networkInterface : Collections.list(NetworkInterface.getNetworkInterfaces())) {
                if (networkInterface.getName().contains("wlan")) {
                    for (InetAddress inetAddress : Collections.list(networkInterface.getInetAddresses())) {
                        if (!inetAddress.isLoopbackAddress() && NetworkUtil.isIPv4Address(inetAddress.getHostAddress())) {
