...
public class ImsMultiEndPointImpl extends IImsMultiEndpoint.Stub {
    ...
    public void setDialogInfo(DialogEvent dialogEvent, int i) {
        ...
        for (Dialog dialog : dialogEvent.getDialogList()) {
            if (dialog != null) {
                ...
                if (!TextUtils.isEmpty(remoteUri) && remoteUri.contains(":")) {
                    ...
                    boolean z = ((i == 2 || i == 4 || i == 8) && !TextUtils.isEmpty(dialog.getRemoteDispName()) && remoteUri.contains(dialog.getRemoteDispName())) ? false : true;
        ...
        for (ImsExternalCallState imsExternalCallState : this.mDialogList) {
            StringBuilder sb = new StringBuilder();
            sb.append("[");
            sb.append(imsExternalCallState.getCallId() % 100000);
            sb.append(",");
            String str = "T";
            sb.append(imsExternalCallState.getCallState() == 1 ? "C" : "T");
            sb.append(",");
            sb.append(imsExternalCallState.isCallHeld() ? "H" : "A");
            sb.append(",");
            if (!imsExternalCallState.isCallPullable()) {
                str = "F";
            }
            sb.append(str);
            sb.append("]");
